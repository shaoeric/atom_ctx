...
AtomCtx multi-tenancy does not mean "deploy one isolated server per team." Instead, a single AtomCtx Server uses three identity boundaries, `account`, `user`, and `agent`, to control sharing and isolation.

This model fits two common scenarios:

- Multiple teams or customers share one AtomCtx service, but their data must stay isolated
...
- Serve multiple teams, customers, or applications from one AtomCtx Server
...
AtomCtx Server supports two multi-tenant related authentication modes:

| Mode | Config | Identity source | Typical use case |
|------|--------|-----------------|------------------|
| `api_key` | `server.auth_mode = "api_key"` | Root key or user key | Standard deployment |
| `trusted` | `server.auth_mode = "trusted"` | Upstream-injected `X-AtomCtx-Account` / `X-AtomCtx-User` | Behind a trusted gateway |

### What `root_api_key` Does

Once `server.root_api_key` is configured, AtomCtx enters formal multi-tenant mode:
...
For users, URIs still look like normal `ctx://...` paths:
...
- The plugin explicitly passes `X-AtomCtx-Agent`
...
- Best for "one OpenClaw instance maps to one AtomCtx user identity"
...
If you give the plugin a root key directly, normal tenant-scoped data APIs will lack `X-AtomCtx-Account` and `X-AtomCtx-User`, so that is not a good default for day-to-day access.
...
- The bot connects to AtomCtx with a root key
