...
AtomCtx 使用 JSON 配置文件（`ctx.conf`）进行设置。配置文件支持 Embedding、VLM、Rerank、存储、解析器等多个模块的配置。

## 快速开始

在项目目录创建 `~/.ctx/ctx.conf`：
...
使用 `input: "multimodal"` 时，AtomCtx 可以嵌入文本、图片（PNG、JPG 等）和混合内容。
...
> **注意：** 需安装 `pip install "google-genai>=1.0.0"`。异步批量嵌入：`pip install "atom-ctx[gemini-async]"`。
...
**依赖**：`pip install 'atom-ctx[bot-feishu]'`
...
| 配置文件 | 用途 | 默认路径 |
|---------|------|---------|
| `ctx.conf` | SDK 嵌入模式 + 服务端配置 | `~/.ctx/ctx.conf` |
| `ctx-cli.conf` | HTTP 客户端和 CLI 连接远程服务端 | `~/.ctx/ctx-cli.conf` |

配置文件放在默认路径时，AtomCtx 自动加载，无需额外设置。
...
本文档上方各配置段（embedding、vlm、rerank、storage）均属于 `ctx.conf`。SDK 嵌入模式和服务端共用此文件。

如需配置 memory 相关行为，可在 `ctx.conf` 中添加 `memory` 段：
...
`agent_scope_mode` 只影响 `ctx://agent/{agent_space}/memories/...` 这类 agent 级命名空间，不影响 `ctx://user/{user_space}/memories/...` 下的 user memory。
...
| `account` | 默认发送为 `X-AtomCtx-Account` 的租户标识 | `null` |
| `user` | 默认发送为 `X-AtomCtx-User` 的用户标识 | `null` |
...
将 AtomCtx 作为 HTTP 服务运行时，在 `ctx.conf` 中添加 `server` 段：
...
`api_key` 模式使用 API Key 认证；`trusted` 模式信任上游网关或受信调用方注入的 `X-AtomCtx-Account` / `X-AtomCtx-User` 请求头。
...
| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `local.key_file` | str | 根密钥文件路径 | `~/.ctx/master.key` |
...
| `vault.key_name` | str | 根密钥名称 | `"atom_ctx-root"` |
