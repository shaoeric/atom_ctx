...
AtomCtx 是为 AI Agent 设计的上下文数据库，将所有上下文（Memory、Resource、Skill）统一抽象为目录结构，支持语义检索和渐进式内容加载。
...
| **PackService** | 导入导出 | export_ctxpack, import_ctxpack |
| **DebugService** | 调试服务 | observer (ObserverService) |

## 双层存储

AtomCtx 采用双层存储架构，实现内容与索引分离（详见 [存储架构](./05-storage.md)）：
