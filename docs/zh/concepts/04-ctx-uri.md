...
Ctx URI 是 AtomCtx 中所有内容的统一资源标识符。
...
摒弃传统的扁平化数据库思维，将所有上下文组织为一套文件系统。Agent 不再仅是通过向量搜索来找数据，而是可以通过确定性的路径和标准文件系统指令来定位和浏览数据。每个上下文或目录分配唯一的 URI 标识字符串，格式为 ctx://{scope}/{path}，让系统能精准定位并访问存储在不同位置的资源。
...
```
ctx://resources/                           # 所有资源
ctx://resources/my-project/                # 项目根目录
ctx://resources/my-project/docs/           # 文档目录
ctx://resources/my-project/docs/api.md     # 具体文件
```
...
```
ctx://user/                                # 用户根目录
ctx://user/memories/                       # 所有用户记忆
ctx://user/memories/preferences/           # 用户偏好
ctx://user/memories/preferences/coding     # 具体偏好
ctx://user/memories/entities/              # 实体记忆
ctx://user/memories/events/                # 事件记忆
```
...
```
ctx://agent/                               # Agent 根目录
ctx://agent/skills/                        # 所有技能
ctx://agent/skills/search-web              # 具体技能
ctx://agent/memories/                      # Agent 记忆
ctx://agent/memories/cases/                # 学习的案例
ctx://agent/memories/patterns/             # 学习的模式
ctx://agent/instructions/                  # Agent 指令
```

### 会话数据

```
ctx://session/{session_id}/                # 会话根目录
ctx://session/{session_id}/messages/       # 会话消息
ctx://session/{session_id}/tools/          # 工具执行
ctx://session/{session_id}/history/        # 归档历史
```
...
# 技能添加到 agent 作用域
await client.add_skill(skill)  # 自动到 ctx://agent/skills/
```
