...
AtomCtx 采用信封加密（Envelope Encryption）架构，使用三层密钥体系：

```
┌─────────────────────────────────────────────────────────┐
│  Layer 1: Root Key（根密钥）                          │
│  • 整个 AtomCtx 实例全局唯一                       │
│  • 存储：KMS 服务 / ~/.ctx/master.key         │
│  • 用途：派生所有账户密钥                              │
└────────────────────┬────────────────────────────────────┘
                     │ HKDF 派生
                     ▼
...
AtomCtx 支持三种密钥提供程序，适应不同的部署场景：

| 提供程序 | 适用场景 | Root Key 存储 | 特点 |
|---------|---------|--------------|------|
| **Local** | 开发环境、单节点部署 | 本地文件 `~/.ctx/master.key` | 简单，无需外部服务 |
...
加密文件使用统一的信封格式，以魔术数 `OVE1`（AtomCtx Encryption v1）开头：
