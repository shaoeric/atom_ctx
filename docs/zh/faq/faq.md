...
AtomCtx 是一个专为 AI Agent 设计的开源上下文数据库。它解决了构建 AI Agent 时的核心痛点：
...
AtomCtx 通过文件系统范式统一管理所有上下文，实现分层供给与自我迭代。

### AtomCtx 和传统向量数据库有什么本质区别？
...
L0/L1/L2 是 AtomCtx 的渐进式内容加载机制，解决了"海量上下文一次性塞入提示词"的问题：
...
Ctx URI 是 AtomCtx 的统一资源标识符，格式为 `ctx://{scope}/{path}`。它让系统能精准定位任何上下文：
...
2. **切换到 HTTP 模式**：在 `ctx.conf` 中设置 `storage.agfs.mode = "http-client"`，并确保有一个正在运行的 `agfs-server`。
...
配置文件放在默认路径 `~/.ctx/ctx.conf` 时自动加载；也可通过环境变量 `CTX_CONFIG_FILE` 或命令行 `--config` 指定其他路径。详见 [配置指南](../guides/01-configuration.md)。
...
SDK 构造函数仅接受 `url`、`api_key`、`path` 参数。其他配置（embedding、vlm 等）通过 `ctx.conf` 配置文件管理。
...
await client.add_resource(
    ...
    target="ctx://resources/docs/"  # 指定存储位置
)
...
会话管理是 AtomCtx 的核心能力，支持对话追踪和记忆提取：
...
2. **Embedding 模型配置错误**
   - 检查 `~/.ctx/ctx.conf` 中的 `api_key` 是否正确
...
4. **异步操作**：充分利用 `AsyncAtomCtx` / `AsyncHTTPClient` 的异步特性
...
是的，AtomCtx 完全开源，采用 Apache 2.0 许可证。

## 相关文档

- [简介](../getting-started/01-introduction.md) - 了解 AtomCtx 的设计理念
