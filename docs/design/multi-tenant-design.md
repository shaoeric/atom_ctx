...
AtomCtx 已定义了 `UserIdentifier(account_id, user_id, agent_id)` 三元组（PR #120），但多租户隔离尚未实施。当前状态：

- **认证**：单一全局 `api_key`，HMAC 比较（`atom_ctx/server/auth.py`）
- **无 RBAC**：所有认证用户拥有完全访问权限
- **无存储隔离**：`VikingFS._uri_to_path` 将 `ctx://` 映射到 `/local/`，无 account_id 前缀
- **VectorDB**：单一 `context` collection，无租户过滤
- **服务层**：`AtomCtxService` 持有单例 `_user`，不支持请求级用户上下文
...
| 类型 | 格式 | 解析结果 | 存储位置 |
|------|------|----------|----------|
| Root Key | `secrets.token_hex(32)` | role=ROOT | `ctx.conf` server 段 |
...
2. alice 用 key 访问 API
   GET /api/v1/fs/ls?uri=ctx://  -H "X-API-Key: 7f3a9c1e..."   → 200 OK
...
- **Root Key**：`ctx.conf` 的 `server` 段（静态配置）
...
**为什么存 AGFS**：User key 是运行时通过 Admin API 动态增删的，不能放 ctx.conf。选择 AGFS 的核心理由是多节点一致性——多个 server 共享同一个 AGFS 后端时，一个节点创建的用户其他节点立即可见。

### 2.4 新模块 `atom_ctx/server/api_keys.py`
...
新建 `atom_ctx/server/identity.py`：
...
class ResolvedIdentity:
    ...
    agent_id: Optional[str] = None  # 来自 X-AtomCtx-Agent header
...
4. 从 `X-AtomCtx-Agent` header 读取 `agent_id`（默认 `"default"`）
5. 构造 `RequestContext(UserIdentifier(account_id, user_id, agent_id), role)`

### 3.3 FastAPI 依赖注入

改动 `atom_ctx/server/auth.py`：

```python
async def resolve_identity(request, x_api_key, authorization, x_atom_ctx_agent) -> ResolvedIdentity
...
新增 Router: `atom_ctx/server/routers/admin.py`
...
**改动文件**: `atom_ctx/storage/ctx_fs.py`
...
```
ctx://user/{user_space}/memories/x + account_id="acme"
...
→ ctx://user/{user_space}/memories/x
```
...
user/agent 级隔离通过**逐层遍历时过滤**实现。用户可以从公共根目录（如 `ctx://resources`）开始遍历，但每一层只能看到自己有权限的条目。

**示例**：

```
# alice（USER 角色）
ls ctx://resources           → 看到 account 内共享的 resources（无 user 隔离）
ls ctx://agent/memories      → 只看到 alice 当前 agent 的 {agent_space}/
ls ctx://user/memories       → 只看到 {alice_user_space}/

# admin（ADMIN 角色）
ls ctx://resources           → 同上，resources 在 account 内共享
ls ctx://user/memories       → 看到所有用户的 space 目录
```
...
def _is_accessible(self, uri: str, ctx: RequestContext) -> bool:
    ...
    # 结构性目录（不含 space，如 ctx://user/memories）→ 允许遍历
...
**改动文件**: `atom_ctx/storage/collection_schemas.py`
...
**改动文件**: `atom_ctx/core/directories.py`

- 创建新账户时，初始化 account 级预设目录结构（公共根：`ctx://user`、`ctx://agent`、`ctx://resources` 等）
- 用户首次访问时，懒初始化 user space 子目录（`ctx://user/{user_space}/memories/preferences` 等）
- agent 首次使用时，懒初始化 agent space 子目录（`ctx://agent/{agent_space}/memories/cases` 等）
...
```
# ACL 记录
{ "grantee_space": "bob_user_space", "granted_uri_prefix": "ctx://resources/{alice_space}/project-x" }
...
```
# 目录记录
{ "uri": "ctx://resources/{alice_space}/project-x", "owner_space": "alice_space", "shared_spaces": ["bob_space"] }
...
**改动文件**: `atom_ctx/server/config.py`
...
| 身份 header | `X-AtomCtx-User` + `X-AtomCtx-Agent` | 仅 `X-AtomCtx-Agent` |

### 7.1 Python SDK

**改动文件**: `atom_ctx_cli/client/http.py`, `atom_ctx_cli/client/sync_http.py`
...
**改动文件**: `atom_ctx_cli/session/user_id.py`

`ctx-cli.conf` 新增 `agent_id` 字段：
...
CLI 发起请求时通过 `X-AtomCtx-Agent` header 携带 agent_id。不再需要配置 `account_id` 和 `user_id`。
...
**新建** `atom_ctx/server/identity.py`，依赖：无
...
from atom_ctx.session.user_id import UserIdentifier
...
**修改** `atom_ctx/server/config.py`，依赖：无
...
**新建** `atom_ctx/server/api_keys.py`，依赖：T1
...
**重写** `atom_ctx/server/auth.py`，依赖：T1, T2, T3

删除现有的 `verify_api_key()`、`get_user_header()`、`get_agent_header()`，替换为：

**`resolve_identity(request, x_api_key, authorization, x_atom_ctx_agent) -> ResolvedIdentity`**：
```
...
4. identity = api_key_manager.resolve(key)
   ...
   - 均不匹配 → 401
5. identity.agent_id = x_atom_ctx_agent or "default"
6. 返回 identity
```
...
**修改** `atom_ctx/server/app.py`，依赖：T2, T4
...
1. `service = AtomCtxService()` → 启动 AGFS
...
**新建** `atom_ctx/server/routers/admin.py`，依赖：T2, T4
...
**修改文件**：`atom_ctx_cli/client/http.py`, `atom_ctx_cli/client/sync_http.py`，依赖：T4

HTTP 模式新增 `agent_id` 参数，通过 `X-AtomCtx-Agent` header 发送：
...
if self._agent_id:
    headers["X-AtomCtx-Agent"] = self._agent_id
```
...
| 文档 | 改动 |
|------|------|
| `guides/01-configuration.md` | server 段 `api_key` → `root_api_key`；ctx-cli.conf 新增 `agent_id` 字段说明 |
...
| 文件 | 改动 |
|------|------|
| `examples/ctx.conf.example` | `api_key` → `root_api_key` |
| `examples/server_client/ctx.conf.example` | 同上 |
| `examples/server_client/client_sync.py` | 新增 `--agent-id` 参数 |
| `examples/server_client/client_async.py` | 新增 `agent_id` 参数 |
| `examples/server_client/client_cli.sh` | 添加 `X-AtomCtx-Agent` header 示例 |
| `examples/server_client/ctx-cli.conf.example` | 新增 `agent_id` 字段 |
...
├── README.md                  # 多租户管理流程说明
├── ctx.conf.example            # 启用 root_api_key 的配置示例
...
**修改** `atom_ctx/storage/ctx_fs.py`，依赖：T1
...
**修改** `atom_ctx/storage/collection_schemas.py`，依赖：无
...
`atom_ctx/core/context.py` 中 `Context` 类需增加两个字段：
...
`AtomCtxService.__init__()` 中删除 `self._user`。
...
```python
class XXXService:
    def set_dependencies(self, ctx_fs, ..., user=None):
        self._ctx_fs = ctx_fs
        self._user = user  # ← 删除

    async def some_method(self, ...):
        # 使用 self._ctx_fs 和 self._user
```

改为：
```python
class XXXService:
    def set_dependencies(self, ctx_fs, ...):  # 去掉 user
        self._ctx_fs = ctx_fs

    async def some_method(self, ..., ctx: RequestContext):  # 加 ctx
        # 使用 self._ctx_fs 和 ctx
```
...
- session 路径变为 `ctx://session/{ctx.user.user_space_name()}/{session_id}`
...
- 资源路径使用 `ctx://resources/...`（account 内共享，无 user_space），技能路径使用 `ctx://agent/skills/{ctx.user.agent_space_name()}/...`
...
- 当前：`export_ctxpack(uri)`, `import_ctxpack(data)`
- 改为：加 `ctx`，传给 VikingFS
...
1. **创建新 account 时**（Admin API T11）→ 初始化该 account 的公共根目录（`ctx://user`、`ctx://agent`、`ctx://resources` 等）
2. **用户首次访问时** → 懒初始化 user space 子目录（`ctx://user/{user_space}/memories/preferences` 等）
3. **agent 首次使用时** → 懒初始化 agent space 子目录（`ctx://agent/{agent_space}/memories/cases` 等）
...
**新建** `atom_ctx/cli/migrate.py`，依赖：T6, T7

提供 `python -m atom_ctx migrate` 命令，将旧版单租户数据迁移到多租户路径结构。
...
| `concepts/04-ctx-uri.md` | URI 在多租户下的 account 作用域说明 |
...
| 文件 | 改动类型 | 阶段 | 说明 |
|------|----------|------|------|
| `atom_ctx/server/identity.py` | **新建** | P1 | Role(ROOT/ADMIN/USER), ResolvedIdentity, RequestContext |
| `atom_ctx/server/api_keys.py` | **新建** | P1 | APIKeyManager（per-account 存储，全局索引） |
| `atom_ctx/server/routers/admin.py` | **新建** | P1 | Admin 管理端点（account/user CRUD、角色管理） |
| `atom_ctx/server/auth.py` | 重写 | P1 | verify_api_key → resolve_identity + require_role + get_request_context |
| `atom_ctx/server/config.py` | 修改 | P1 | api_key → root_api_key |
| `atom_ctx/server/app.py` | 修改 | P1 | 初始化 APIKeyManager，注册 Admin Router |
| `atom_ctx_cli/client/http.py` | 修改 | P1 | 新增 agent_id 参数 |
| `atom_ctx_cli/client/sync_http.py` | 修改 | P1 | 新增 agent_id 参数 |
| `atom_ctx/server/routers/*.py` | 修改 | P1+P2 | P1: 迁移到 get_request_context；P2: ctx 传递给 service |
| `atom_ctx/storage/ctx_fs.py` | 修改 | P2 | 方法加 ctx 参数，_uri_to_path 加 account_id 前缀 |
| `atom_ctx/storage/collection_schemas.py` | 修改 | P2 | context collection 加 account_id + owner_space 字段 |
| `atom_ctx/retrieve/hierarchical_retriever.py` | 修改 | P2 | 查询注入 account_id + owner_space 多级过滤 |
| `atom_ctx/service/core.py` | 修改 | P2 | 去除单例 _user，传递 RequestContext |
| `atom_ctx/service/*.py` | 修改 | P2 | 各 sub-service 接受 RequestContext |
| `atom_ctx/core/directories.py` | 修改 | P2 | 按 account 初始化目录 |
| `atom_ctx/core/context.py` | 修改 | P2 | 新增 account_id、owner_space 字段 |
| `atom_ctx/client/local.py` | 修改 | P2 | 支持 UserIdentifier 参数（嵌入模式多租户） |
| `atom_ctx_cli/session/user_id.py` | 修改 | P2 | 新增 user_space_name() 和 agent_space_name() 方法 |
| `atom_ctx/cli/migrate.py` | **新建** | P2 | 数据迁移脚本 |
...
| `examples/ctx.conf.example` | 修改 | P1 | `api_key` → `root_api_key` |
| `examples/server_client/ctx.conf.example` | 修改 | P1 | 同上 |
...
10. **配置简化**：`ctx.conf` server 段移除 `private_key` 和 `multi_tenant`，仅保留 `root_api_key` 和 `cors_origins`。
...
- **T15**：数据迁移脚本（`python -m atom_ctx migrate`），将旧版单租户数据迁移到多租户路径结构，Phase 2 实现
