...
- [#578](https://github.com/volcengine/atom-ctx/issues/578) "[Feature]: 允许提示词模板自定义添加和指定" - Current AtomCtx memory_extractor uses 8 fixed memory categories (profile, preferences, entities, events, cases, patterns, tools, skills), adding new memory types requires modifying multiple core code files, prompt templates and merge logic are hard-coded. This design implements the custom template capability requested in #578. / [#578](https://github.com/volcengine/atom-ctx/issues/578) "[Feature]: 允许提示词模板自定义添加和指定" - 当前 AtomCtx 的 memory_extractor 使用 8 个固定的记忆类别（profile, preferences, entities, events, cases, patterns, tools, skills），新增记忆类型需要修改多处核心代码，提示模板和合并逻辑都是硬编码的。本设计实现了 #578 中要求的自定义模板能力。
...
Implement a memory templating system based on AtomCtx storage, such that: / 实现基于 AtomCtx 存储的记忆模版化系统，使得：
...
- Memory templating (AtomCtx's memory template is a subset of the commercial version, preserving the possibility of future upgrade to commercial version) / 记忆模版化（atom_ctx的记忆模版是商业化版本的一个子集，保留未来升级到商业化版本的可能性）
...
directory: "ctx://user/{user_space}/memories"
...
directory: "ctx://user/{user_space}/memories/entities"
...
fields:
  - name: entity_name
    ...
    description: |
      ...
      Should be the specific name of the entity such as "AtomCtx project", "Alice (colleague)", "Redis", etc.
    merge_op: immutable
  ...
  - name: content
    ...
    description: |
      ...
      Example: "AtomCtx is an AI Agent long-term memory management system the user is developing. The project uses Python and AGFS tech stack, core features include memory extraction, deduplication, and retrieval. Currently in active development, goal is to build Claude-like long-term memory capabilities."
    merge_op: patch
```
...
directory: "ctx://user/{user_space}/memories/events"
...
fields:
  - name: event_name
    ...
    description: |
      ...
      Should be a specific event description such as "Decided to refactor memory system", "Started AtomCtx project", "Completed Q3 review", etc.
...
directory: "ctx://agent/{agent_space}/memories/cases"
...
directory: "ctx://agent/{agent_space}/memories/patterns"
...
directory: "ctx://agent/{agent_space}/memories/skills"
...
│  │  - Write to AtomCtx Storage (L0/L1/L2)                 │  │
...
- description
- directory: Full URI for memory data storage, e.g., "ctx://user/{user_space}/memories/preferences" / 记忆数据存放的完整 URI，如 "ctx://user/{user_space}/memories/preferences"
...
**Available Tools** (consistent with AtomCtx VikingFS API) / **可使用的工具**（与 AtomCtx VikingFS API 保持一致）:
...
**tree**
- Parameters / 参数: `uri: str = "ctx://", output: str = "agent", abs_limit: int = 256, show_all_hidden: bool = False, node_limit: int = 1000, level_limit: int = 3`
...
- load_from_dir(dir_path): Load all MemoryType YAML files from directory (one type per file) / 从目录加载所有 MemoryType YAML 文件（每个文件一个类型）
  - Default load built-in types from `atom_ctx/session/memory/schemas/` / 默认从 `atom_ctx/session/memory/schemas/` 加载内置类型
...
1. **ls**: Get all memory directory structures / 获取所有记忆目录结构
   - `ctx://user/{user_space}/memories/` and subdirectories / 及子目录
   - `ctx://agent/{agent_space}/memories/` and subdirectories / 及子目录
...
- **URI Structure**:
  - User: `ctx://user/{space}/memories/{category}/`
  - Agent: `ctx://agent/{space}/memories/{category}/`
...
| File / 文件                                                         | Purpose / 作用                                                  |
| ----------------------------------------------------------------- | ------------------------------------------------------------- |
| `atom_ctx/session/memory_extractor.py`                          | Memory extraction main logic (~1200 lines) / 记忆提取主逻辑 (~1200行) |
| `atom_ctx/session/memory_deduplicator.py`                       | Deduplication decision (~395 lines) / 去重决策 (~395行)            |
| `atom_ctx/session/compressor.py`                                | Session compressor (~447 lines) / 会话压缩器 (~447行)               |
| `atom_ctx/prompts/templates/compression/memory_extraction.yaml` | Extraction prompt template (~400 lines) / 提取提示模板 (~400行)      |
...
1. Create `atom_ctx/session/memory/memory_data.py` / 创建 `atom_ctx/session/memory/memory_data.py`
...
2. Create `atom_ctx/session/memory/memory_operations.py` / 创建 `atom_ctx/session/memory/memory_operations.py`
   - WriteOp, EditOp, DeleteOp
   - MemoryOperations (LLM final output format) / MemoryOperations（LLM 最终输出格式）

3. Create `atom_ctx/session/memory/memory_react.py` / 创建 `atom_ctx/session/memory/memory_react.py`
   - Standard LLM ReAct implementation / 标准的 LLM ReAct 实现

4. Create `atom_ctx/session/memory/memory_functions.py` / 创建 `atom_ctx/session/memory/memory_functions.py`
   - ReadArgs/ReadResult
   - FindArgs/FindResult
   - LsArgs/LsResult
...
5. Create `atom_ctx/session/memory/memory_patch.py` / 创建 `atom_ctx/session/memory/memory_patch.py`
...
6. Create `atom_ctx/session/memory/memory_types.py` / 创建 `atom_ctx/session/memory/memory_types.py`
...
7. Create YAML config files / 创建 YAML 配置文件
   - Place in `atom_ctx/session/memory/schemas/` directory / 放在 `atom_ctx/session/memory/schemas/` 目录
   - preferences.yaml, entities.yaml, tools.yaml, etc. / preferences.yaml、entities.yaml、tools.yaml 等

### Phase 4: MemoryUpdater

8. Create `atom_ctx/session/memory/memory_updater.py` / 创建 `atom_ctx/session/memory/memory_updater.py`
   - MemoryUpdater class / MemoryUpdater 类
   - apply_operations() method - system direct execution / apply_operations() 方法 - 系统直接执行
   - Integration with AtomCtx Storage / 与 AtomCtx Storage 集成

### Phase 5: ReAct Orchestrator / ReAct 编排器

9. Complete `atom_ctx/session/memory/memory_react.py` / 完善 `atom_ctx/session/memory/memory_react.py`
...
└── schemas/                # Config directory / 配置目录
    ├── profile.yaml       # directory: ctx://user/{user_space}/memories
    ├── preferences.yaml   # directory: ctx://user/{user_space}/memories/preferences
    ├── entities.yaml      # directory: ctx://user/{user_space}/memories/entities
    ├── events.yaml        # directory: ctx://user/{user_space}/memories/events
    ├── cases.yaml         # directory: ctx://agent/{agent_space}/memories/cases
    ├── patterns.yaml      # directory: ctx://agent/{agent_space}/memories/patterns
    ├── tools.yaml         # directory: ctx://agent/{agent_space}/memories/tools
    └── skills.yaml        # directory: ctx://agent/{agent_space}/memories/skills
...
```
atom_ctx/session/memory_extractor.py  # Preserve existing version, do not modify / 保留现有版本，不修改
```
...
**Execution** / **执行**: MemoryUpdater.apply_operations() directly applies patch, writes to AtomCtx Storage / MemoryUpdater.apply_operations() 直接应用 patch，写入 AtomCtx Storage
...
- ✅ Fully compatible with AtomCtx existing storage structure / 完全兼容 AtomCtx 现有存储结构
