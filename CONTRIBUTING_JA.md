...
AtomCtxに興味をお持ちいただきありがとうございます！あらゆる種類のコントリビューションを歓迎します：
...
- **Rust**: 1.88以上（ソースビルド時に同梱の `ctx` CLI もビルドされるため必須）
...
AtomCtxは以下の環境向けにプリコンパイル済み**Wheel**パッケージを提供しています：
...
AtomCtxはAGFSに対してデフォルトで`binding-client`モードを使用し、事前にビルドされたネイティブ成果物を利用します。**AGFS（Go）**コード、同梱の**Rust CLI**、または**C++拡張**を変更した場合や、プリビルド成果物が見つからない場合は、再コンパイルと再インストールが必要です。プロジェクトルートで以下のコマンドを実行してください：

```bash
uv pip install -e . --force-reinstall
```

このコマンドにより`setup.py`が再実行され、AGFS、同梱 `ctx` CLI、C++コンポーネントの再ビルドがトリガーされます。

### 3. 環境設定

設定ファイル `~/.ctx/ctx.conf` を作成します：
...
Rust CLI（`ctx`）は、AtomCtx Serverとやり取りするための高性能コマンドラインクライアントを提供します。

`ctx` を直接使わない場合でも、AtomCtx をソースからビルドするなら Rust ツールチェーンは必要です。パッケージング時に同梱 CLI バイナリも一緒にビルドされるためです。
...
インストール後、`ctx --help`を実行して利用可能なすべてのコマンドを確認できます。CLI接続設定は`~/.ctx/ctx-cli.conf`に記述します。
...
│
├── atom_ctx/           # Python SDK
│   ├── async_client.py   # AsyncAtomCtxクライアント
│   ├── sync_client.py    # SyncAtomCtxクライアント
...
共通フィクスチャは`tests/conftest.py`に定義されており、`client`（初期化済み`AsyncAtomCtx`）、`uninitialized_client`、`temp_dir`、`sample_markdown_file` などが含まれます。
