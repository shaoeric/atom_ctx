...
from atom_ctx.storage.expr import (
    And,
    Contains,
    Eq,
    FilterExpr,
...
from atom_ctx.storage.vectordb.collection.result import FetchDataInCollectionResult
from atom_ctx_cli.utils import get_logger
from atom_ctx_cli.utils.config.vectordb_config import DEFAULT_INDEX_NAME
...
class CollectionAdapter(ABC):
    ...
    def _encode_uri_field_value(value: Any) -> Any:
        ...
        if not stripped.startswith("ctx://"):
            return value
        suffix = stripped[len("ctx://") :].strip("/")
    ...
    def _decode_uri_field_value(value: Any) -> Any:
        ...
        return f"ctx://{suffix}" if suffix else "ctx://"
