...
from atom_ctx.parse.parsers.constants import (
    CODE_EXTENSIONS,
...
from atom_ctx.parse.parsers.media.utils import (
    generate_audio_summary,
    generate_image_summary,
    generate_video_summary,
    get_media_type,
)
from atom_ctx.prompts import render_prompt
from atom_ctx.server.identity import RequestContext, Role
from atom_ctx.storage.queuefs.named_queue import DequeueHandlerBase
...
from atom_ctx.telemetry import bind_telemetry, resolve_telemetry
...
from atom_ctx_cli.utils import CtxURI
from atom_ctx_cli.utils.config import get_atom_ctx_config
from atom_ctx_cli.utils.logger import get_logger
...
class SemanticProcessor(DequeueHandlerBase):
    ...
    async def _check_file_content_changed(
        ...
        ctx_fs = get_ctx_fs()
        try:
            current_stat = await ctx_fs.stat(file_path, ctx=ctx)
            target_stat = await ctx_fs.stat(target_file, ctx=ctx)
            ...
            if current_size is not None and target_size is not None and current_size != target_size:
                return True
            current_content = await ctx_fs.read_file(file_path, ctx=ctx)
            target_content = await ctx_fs.read_file(target_file, ctx=ctx)
    ...
    async def _reenqueue_semantic_msg(self, msg: SemanticMsg) -> None:
        ...
        from atom_ctx.storage.queuefs import get_queue_manager
        ...
        try:
            ...
            with telemetry_ctx:
                ...
                if msg.context_type == "memory":
                    ...
                    ctx_fs = get_ctx_fs()
                    if msg.target_uri:
                        target_exists = await ctx_fs.exists(
                    ...
                    if msg.lifecycle_lock_handle_id:
                        ...
                        msg.lifecycle_lock_handle_id = await self._ensure_lifecycle_lock(
                            ...
                            ctx_fs._uri_to_path(lock_uri, ctx=self._current_ctx),
                        )
            ...
            if msg and msg.lifecycle_lock_handle_id:
                try:
                    from atom_ctx.storage.transaction import get_lock_manager
    ...
    async def _process_memory_directory(self, msg: SemanticMsg) -> None:
        ...
        try:
            entries = await ctx_fs.ls(dir_uri, ctx=ctx)
        except Exception as e:
        ...
        for entry in entries:
            ...
            if not entry.get("isDir", False):
                item_uri = CtxURI(dir_uri).join(name).uri
        ...
        if msg.changes:
            try:
                old_overview = await ctx_fs.read_file(f"{dir_uri}/.overview.md", ctx=ctx)
                if old_overview:
        ...
        try:
            await ctx_fs.write_file(f"{dir_uri}/.overview.md", overview, ctx=ctx)
            await ctx_fs.write_file(f"{dir_uri}/.abstract.md", abstract, ctx=ctx)
    ...
    async def _sync_topdown_recursive(
        ...
        async def list_children(dir_uri: str) -> Tuple[Dict[str, str], Dict[str, str]]:
            ...
            dirs: Dict[str, str] = {}
            try:
                entries = await ctx_fs.ls(dir_uri, show_all_hidden=True, ctx=ctx)
            except Exception as e:
        ...
        async def sync_dir(root_dir: str, target_dir: str) -> None:
            ...
            try:
                await ctx_fs._mv_vector_store_l0_l1(root_dir, target_dir, ctx=ctx)
            except Exception as e:
                logger.error(
            ...
            for name in sorted(file_names):
                ...
                if root_file and name in target_dirs:
                    ...
                    try:
                        await ctx_fs.rm(target_conflict_dir, recursive=True, ctx=ctx)
                ...
                if target_file and name in root_dirs and not root_file:
                    try:
                        await ctx_fs.rm(target_file, ctx=ctx)
                ...
                if root_file and target_file:
                    ...
                    if changed:
                        ...
                        try:
                            await ctx_fs.mv(root_file, target_file, ctx=ctx)
                        except Exception as e:
                            logger.error(
                ...
                if root_file and not target_file:
                    ...
                    target_file_uri = CtxURI(target_dir).join(name).uri
                    try:
                        await ctx_fs.mv(root_file, target_file_uri, ctx=ctx)
                    except Exception as e:
                        logger.error(
            ...
            for name in sorted(dir_names):
                ...
                if root_subdir and name in target_files:
                    ...
                    try:
                        await ctx_fs.rm(target_conflict_file, ctx=ctx)
                ...
                if target_subdir and not root_subdir:
                    try:
                        await ctx_fs.rm(target_subdir, recursive=True, ctx=ctx)
                ...
                if root_subdir and not target_subdir:
                    ...
                    target_subdir_uri = CtxURI(target_dir).join(name).uri
                    try:
                        await ctx_fs.mv(root_subdir, target_subdir_uri, ctx=ctx)
                    except Exception as e:
                        logger.error(
        ...
        target_exists = await ctx_fs.exists(target_uri, ctx=ctx)
        if not target_exists:
            parent_uri = CtxURI(target_uri).parent
            if parent_uri:
                await ctx_fs.mkdir(parent_uri.uri, exist_ok=True, ctx=ctx)
            diff.added_dirs.append(root_uri)
            await ctx_fs.mv(root_uri, target_uri, ctx=ctx)
            return diff

        await sync_dir(root_uri, target_uri)
        try:
            await ctx_fs.delete_temp(root_uri, ctx=ctx)
        except Exception as e:
    ...
    async def _collect_children_abstracts(
        ...
        for child_uri in children_uris:
            abstract = await ctx_fs.abstract(child_uri, ctx=ctx)
    ...
    async def _generate_text_summary(
        ...
        ctx_fs = get_ctx_fs()
        vlm = get_atom_ctx_config().vlm
        active_ctx = ctx or self._current_ctx

        content = await ctx_fs.read_file(file_path, ctx=active_ctx)
        ...
        max_chars = get_atom_ctx_config().semantic.max_file_content_chars
        ...
        if file_type == FILE_TYPE_CODE:
            code_mode = get_atom_ctx_config().code.code_summary_mode

            if code_mode in ("ast", "ast_llm") and len(content.splitlines()) >= 100:
                from atom_ctx.parse.parsers.code.ast import extract_skeleton

                verbose = code_mode == "ast_llm"
                skeleton_text = extract_skeleton(file_name, content, verbose=verbose)
                if skeleton_text:
                    max_skeleton_chars = get_atom_ctx_config().semantic.max_skeleton_chars
    ...
    def _enforce_size_limits(self, overview: str, abstract: str) -> Tuple[str, str]:
        ...
        semantic = get_atom_ctx_config().semantic
    ...
    async def _vectorize_directory(
        ...
        from atom_ctx.utils.embedding_utils import vectorize_directory_meta
    ...
    async def _vectorize_single_file(
        ...
        from atom_ctx.utils.embedding_utils import vectorize_file
