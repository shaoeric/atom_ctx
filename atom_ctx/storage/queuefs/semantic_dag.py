...
class SemanticDagExecutor:
    ...
    async def _list_dir(self, uri: str) -> tuple[list[str], list[str]]:
        ...
        try:
            entries = await self._ctx_fs.ls(uri, ctx=self._ctx)
        except Exception as e:
        ...
        for entry in entries:
            ...
            item_uri = CtxURI(uri).join(name).uri
    ...
    async def _check_file_content_changed(self, file_path: str) -> bool:
        ...
        if not target_path:
            return True
        try:
            current_stat = await self._ctx_fs.stat(file_path, ctx=self._ctx)
            target_stat = await self._ctx_fs.stat(target_path, ctx=self._ctx)
            ...
            if current_size is not None and target_size is not None and current_size != target_size:
                return True
            current_content = await self._ctx_fs.read_file(file_path, ctx=self._ctx)
            target_content = await self._ctx_fs.read_file(target_path, ctx=self._ctx)
            ...
            if parent_uri not in self._overview_cache:
                async with self._overview_cache_lock:
                    if parent_uri not in self._overview_cache:
                        ...
                        overview_content = await self._ctx_fs.read_file(
    ...
    async def _read_existing_overview_abstract(
        ...
        if not target_path:
            return None, None
        try:
            overview = await self._ctx_fs.read_file(f"{target_path}/.overview.md", ctx=self._ctx)
            abstract = await self._ctx_fs.read_file(f"{target_path}/.abstract.md", ctx=self._ctx)
    ...
    async def _overview_task(self, dir_uri: str) -> None:
        ...
        try:
            ...
            try:
                await self._ctx_fs.write_file(f"{dir_uri}/.overview.md", overview, ctx=self._ctx)
                await self._ctx_fs.write_file(f"{dir_uri}/.abstract.md", abstract, ctx=self._ctx)
            except Exception:
