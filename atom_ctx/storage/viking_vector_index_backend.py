...
"""VikingDB storage backend for AtomCtx."""
...
from atom_ctx.storage.expr import And, Eq, FilterExpr, In, Or, PathScope, RawDSL
from atom_ctx.storage.vectordb.collection.collection import Collection
from atom_ctx.storage.vectordb.utils.logging_init import init_cpp_logging
from atom_ctx.storage.vectordb_adapters import create_collection_adapter
from atom_ctx_cli.utils import get_logger
from atom_ctx_cli.utils.config.vectordb_config import DEFAULT_INDEX_NAME, VectorDBBackendConfig
...
class VikingVectorIndexBackend:
    ...
    async def delete_uris(self, ctx: RequestContext, uris: List[str]) -> None:
        for uri in uris:
            ...
            if ctx.role == Role.USER and uri.startswith(("ctx://user/", "ctx://agent/")):
                owner_space = (
