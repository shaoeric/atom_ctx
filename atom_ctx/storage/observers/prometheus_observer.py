...
"""PrometheusObserver: Prometheus metrics exporter for AtomCtx."""

import threading
from threading import Lock
from typing import Dict, Iterable, List, Optional, Tuple

from atom_ctx.storage.observers.base_observer import BaseObserver
...
class PrometheusObserver(BaseObserver):
    ...
    def render_metrics(self) -> str:
        ...
        lines.extend(
            [
                "# HELP atom_ctx_retrieval_requests_total Total retrieval requests.",
                "# TYPE atom_ctx_retrieval_requests_total counter",
                f"atom_ctx_retrieval_requests_total {retrieval_requests_total}",
                "# HELP atom_ctx_embedding_requests_total Total embedding requests.",
                "# TYPE atom_ctx_embedding_requests_total counter",
                f"atom_ctx_embedding_requests_total {embedding_requests_total}",
                "# HELP atom_ctx_vlm_calls_total Total VLM calls.",
                "# TYPE atom_ctx_vlm_calls_total counter",
                f"atom_ctx_vlm_calls_total {vlm_calls_total}",
            ]
        )

        self._append_histogram(
            lines=lines,
            name="atom_ctx_retrieval_latency_seconds",
            help_text="Retrieval request latency in seconds.",
            histogram=retrieval_hist,
        )
        self._append_histogram(
            lines=lines,
            name="atom_ctx_embedding_latency_seconds",
            help_text="Embedding request latency in seconds.",
            histogram=embedding_hist,
        )
        self._append_histogram(
            lines=lines,
            name="atom_ctx_vlm_call_duration_seconds",
