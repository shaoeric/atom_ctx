...
def _validate_forced_variant(
    ...
    if is_x86 and requested == "native":
        raise ImportError("CTX_ENGINE_VARIANT=native is only valid on non-x86 platforms")

    if not is_x86 and requested != "native":
        raise ImportError(
            f"CTX_ENGINE_VARIANT={requested} is not valid on non-x86 platforms; use native"
        )

    if requested not in _BACKEND_MODULES:
        raise ImportError(f"Unknown CTX_ENGINE_VARIANT={requested}")
...
def _select_variant() -> tuple[str | None, tuple[str, ...], str | None]:
    ...
    requested = _normalize_requested_variant(os.environ.get("CTX_ENGINE_VARIANT"))
