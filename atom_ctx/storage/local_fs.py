...
from atom_ctx.storage.queuefs import EmbeddingQueue, get_queue_manager
...
def _validate_ctxpack_member_path(zip_path: str, base_name: str) -> str:
    """Validate a zip member path for ctxpack imports and reject unsafe entries."""
    if not zip_path:
        raise ValueError("Invalid ctxpack entry: empty path")
    if "\\" in zip_path:
        raise ValueError(f"Unsafe ctxpack entry path: {zip_path!r}")
    ...
    if not parts or parts[0] != base_name:
        raise ValueError(f"Invalid ctxpack entry root: {zip_path!r}")

    return zip_path


def ensure_ctxpack_extension(path: str) -> str:
    """Ensure path ends with .ctxpack extension."""
    if not path.endswith(".ctxpack"):
        return path + ".ctxpack"
    return path
...
def get_ctxpack_zip_path(base_name: str, rel_path: str) -> str:
...
def get_ctx_rel_path_from_zip(zip_path: str) -> str:
...
async def _enqueue_direct_vectorization(ctx_fs, uri: str, ctx: RequestContext) -> None:
    queue_manager = get_queue_manager()
    embedding_queue = cast(
        EmbeddingQueue, queue_manager.get_queue(queue_manager.EMBEDDING, allow_create=True)
    )

    parent_uri = CtxURI(uri).parent.uri
    abstract = await ctx_fs.abstract(uri, ctx=ctx)
    resource = Context(
        uri=uri,
...
async def import_ctxpack(
    ...
    """
    Import .ctxpack file to the specified parent path.

    Args:
        ctx_fs: VikingFS instance
        file_path: Local .ctxpack file path
        parent: Target parent URI (e.g., ctx://resources/...)
    ...
    with zipfile.ZipFile(file_path, "r") as zf:
        ...
        if not base_name:
            raise ValueError("Could not determine root directory name from ctxpack")

        root_uri = f"{parent}/{base_name}"

        # 2. Conflict check
        try:
            await ctx_fs.ls(root_uri, ctx=ctx)
            if not force:
        ...
        for info in infolist:
            ...
            safe_zip_path = _validate_ctxpack_member_path(zip_path, base_name)
            # Normalize path separators to handle Windows-created ZIPs
            safe_zip_path = safe_zip_path.replace("\\", "/")

            # Handle directory entries
            if safe_zip_path.endswith("/"):
                rel_path = get_ctx_rel_path_from_zip(safe_zip_path.rstrip("/"))
                target_dir_uri = f"{root_uri}/{rel_path}" if rel_path else root_uri
                await ctx_fs.mkdir(target_dir_uri, exist_ok=True, ctx=ctx)
                continue

            # Handle file entries
            rel_path = get_ctx_rel_path_from_zip(safe_zip_path)
            target_file_uri = f"{root_uri}/{rel_path}" if rel_path else root_uri

            try:
                data = zf.read(safe_zip_path)
                await ctx_fs.write_file_bytes(target_file_uri, data, ctx=ctx)
    ...
    if vectorize:
        await _enqueue_direct_vectorization(ctx_fs, root_uri, ctx=ctx)
        logger.info(f"[local_fs] Enqueued direct vectorization for: {root_uri}")

    return root_uri


async def export_ctxpack(ctx_fs, uri: str, to: str, ctx: RequestContext) -> str:
    """
    Export the specified context path as a .ctxpack file.

    Args:
        ctx_fs: VikingFS instance
        uri: Ctx URI
        to: Target file path (can be an existing directory or a path ending with .ctxpack)
    ...
    if os.path.isdir(to):
        to = os.path.join(to, f"{base_name}.ctxpack")
    else:
        to = ensure_ctxpack_extension(to)

    ensure_dir_exists(to)

    entries = await ctx_fs.tree(uri, show_all_hidden=True, ctx=ctx)
    ...
    with zipfile.ZipFile(to, "w", zipfile.ZIP_DEFLATED, allowZip64=True) as zf:
        ...
        for entry in entries:
            ...
            zip_path = get_ctxpack_zip_path(base_name, rel_path)
            ...
            if entry.get("isDir"):
                ...
                try:
                    data = await ctx_fs.read_file_bytes(full_uri, ctx=ctx)
