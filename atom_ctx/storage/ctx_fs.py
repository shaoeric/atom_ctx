...
"""
VikingFS: AtomCtx file system abstraction layer

Encapsulates AGFSClient, providing file operation interface based on Ctx URI.
Responsibilities:
- URI conversion (ctx:// <-> /local/)
...
from atom_ctx.utils.time_utils import format_simplified, get_current_timestamp, parse_iso_datetime
...
def init_ctx_fs(
    ...
    if enable_recorder:
        _enable_ctx_fs_recorder(_instance)

    return _instance


def _enable_ctx_fs_recorder(ctx_fs: "VikingFS") -> None:
    """
    Enable recorder for a VikingFS instance.

    This wraps the VikingFS instance with recording capabilities.
    Called automatically when enable_recorder=True in init_ctx_fs.

    Args:
        ctx_fs: VikingFS instance to enable recording for
    """
    from atom_ctx.eval.recorder import RecordingVikingFS, get_recorder

    recorder = get_recorder()
    if not recorder.enabled:
        from atom_ctx.eval.recorder import init_recorder

        init_recorder(enabled=True)

    global _instance
    _instance = RecordingVikingFS(ctx_fs)
    logger.info("[VikingFS] IO Recorder enabled")


def enable_ctx_fs_recorder() -> None:
    """
    Enable recorder for the global VikingFS singleton.

    This function wraps the existing VikingFS's AGFS client with recording.
    Must be called after init_ctx_fs().
    """
    global _instance
    if _instance is None:
        raise RuntimeError("VikingFS not initialized. Call init_ctx_fs() first.")
    _enable_ctx_fs_recorder(_instance)


def get_ctx_fs() -> "VikingFS":
...
class VikingFS:
    """AGFS-based AtomCtx file system.
    ...
    def _normalize_uri(uri: str) -> str:
        """Normalize short-format URIs to the canonical ctx:// form."""
        if uri.startswith("ctx://"):
            return uri
        return CtxURI.normalize(uri)
    ...
    def _normalized_uri_parts(cls, uri: str) -> tuple[str, List[str]]:
        ...
        parts = [p for p in normalized[len("ctx://") :].strip("/").split("/") if p]
    ...
    async def rm(
        ...
        """
        from atom_ctx.storage.errors import LockAcquisitionError, ResourceBusyError
    ...
    async def mv(
        ...
        """
        from atom_ctx.pyagfs.helpers import cp as agfs_cp
        from atom_ctx.storage.transaction import LockContext, get_lock_manager
        ...
        async with LockContext(
            ...
            is_temp = old_uri.startswith("ctx://temp/")
    ...
    async def tree(
        ...
        output="original"
        [{'name': '.abstract.md', 'size': 100, 'mode': 420, 'modTime': '2026-02-11T16:52:16.256334192+08:00', 'isDir': False, 'meta': {...}, 'rel_path': '.abstract.md', 'uri': 'ctx://resources...'}]

        output="agent"
        [{'name': '.abstract.md', 'size': 100, 'modTime': '2026-02-11 16:52:16', 'isDir': False, 'rel_path': '.abstract.md', 'uri': 'ctx://resources...', 'abstract': "..."}]
        """
        self._ensure_access(uri, ctx)
        if output == "original":
    ...
    async def find(
        ...
        from atom_ctx_cli.retrieve import (
            ContextType,
            FindResult,
            TypedQuery,
        )

        if target_uri and target_uri not in {"/", "ctx://"}:
            self._ensure_access(target_uri, ctx)

        storage = self._get_vector_store()
        if not storage:
            raise RuntimeError("Vector store not initialized. Call AtomCtx.initialize() first.")
    ...
    async def search(
        ...
        if primary_target_uri and primary_target_uri not in {"/", "ctx://"}:
    ...
    def _uri_to_path(self, uri: str, ctx: Optional[RequestContext] = None) -> str:
        ...
        Pure prefix replacement: ctx://{remainder} -> /local/{account_id}/{remainder}.
    ...
    def _ls_entries(self, path: str) -> List[Dict[str, Any]]:
        ...
        if len(parts) == 2 and parts[0] == "local":
            return [e for e in entries if e.get("name") in CtxURI.VALID_SCOPES]
    ...
    def _path_to_uri(self, path: str, ctx: Optional[RequestContext] = None) -> str:
        ...
        Pure prefix replacement: strips /local/{account_id}/ and prepends ctx://.
    ...
    def _extract_space_from_uri(self, uri: str) -> Optional[str]:
        ...
        URIs are WYSIWYG: ctx://{scope}/{space}/...
    ...
    def _infer_context_type(self, uri: str):
        ...
        from atom_ctx_cli.retrieve import ContextType
    ...
    async def _mv_vector_store_l0_l1(
        ...
        real_ctx = self._ctx_or_default(ctx)
        old_dir = CtxURI.normalize(old_uri).rstrip("/")
        new_dir = CtxURI.normalize(new_uri).rstrip("/")
        if old_dir == new_dir:
            return
    ...
    async def ls(
        ...
        output="original"
        [{'name': '.abstract.md', 'size': 100, 'mode': 420, 'modTime': '2026-02-11T16:52:16.256334192+08:00', 'isDir': False, 'meta': {'Name': 'localfs', 'Type': 'local', 'Content': None}, 'uri': 'ctx://resources/.abstract.md'}]

        output="agent"
        [{'name': '.abstract.md', 'size': 100, 'modTime': '2026-02-11(or 16:52:16 for today)', 'isDir': False, 'uri': 'ctx://resources/.abstract.md', 'abstract': "..."}]
        """
        self._ensure_access(uri, ctx)
        if output == "original":
