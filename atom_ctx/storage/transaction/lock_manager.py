...
class LockManager:
    ...
    async def _redo_session_memory(self, info: Dict[str, Any]) -> None:
        ...
        ctx_fs = get_ctx_fs()
        agfs_path = ctx_fs._uri_to_path(messages_uri, ctx=ctx)
        messages = []
        try:
        ...
        if messages:
            ...
            try:
                from atom_ctx.session import create_session_compressor
    ...
    async def _enqueue_semantic(self, **params: Any) -> None:
        ...
        from atom_ctx.storage.queuefs.semantic_queue import SemanticQueue
