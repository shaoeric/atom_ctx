...
const SESSION_KEY = "ctx_console_api_key";
const THEME_MODE_KEY = "ctx_console_theme_mode";
const NAV_COLLAPSED_KEY = "ctx_console_nav_collapsed";
const RESULT_COLLAPSED_KEY = "ctx_console_result_collapsed_v2";
...
function bindFind() {
  elements.findBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const payload = await callConsole("/ctx/search/find", {
        method: "POST",
  ...
  elements.addResourceSubmitBtn.addEventListener("click", async () => {
    ...
    try {
      if (state.addResourceMode === "path") {
        ...
        const payload = await callConsole("/ctx/resources", {
          method: "POST",
          body: JSON.stringify({
      ...
      const uploadPayload = await callConsole("/ctx/resources/temp_upload", {
        method: "POST",
        body: formData,
      });
      ...
      const addPayload = await callConsole("/ctx/resources", {
        method: "POST",
        body: JSON.stringify({
          ...buildAddResourcePayload(),
...
async function loadTenantAccounts({ showOutput = true } = {}) {
  const payload = await callConsole("/ctx/admin/accounts", { method: "GET" });
...
function bindAddMemory() {
  elements.addMemoryBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const sessionPayload = await callConsole("/ctx/sessions", {
        method: "POST",
        body: JSON.stringify({}),
      });
...
function bindTenants() {
  ...
  elements.tenantCreateAccountBtn.addEventListener("click", async () => {
    ...
    await executeTenantAction({
      ...
      run: async () =>
        callConsole("/ctx/admin/accounts", {
          method: "POST",
...
function bindMonitor() {
  elements.systemBtn.addEventListener("click", async () => {
    try {
      const payload = await callConsole("/ctx/system/status", { method: "GET" });
  ...
  elements.observerBtn.addEventListener("click", async () => {
    try {
      const payload = await callConsole("/ctx/observer/system", { method: "GET" });
