...
from atom_ctx.eval.recorder import IORecord, IOType
...
class RecordAnalysisStats:
    ...
    Attributes:
        ...
        ctx_fs_stats: Detailed VikingFS statistics
    ...
    ctx_fs_stats: Optional[VikingFSStats] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.ctx_fs_stats:
            result["ctx_fs_stats"] = {
                "total_operations": self.ctx_fs_stats.total_operations,
                "success_count": self.ctx_fs_stats.success_count,
                "error_count": self.ctx_fs_stats.error_count,
                "success_rate_percent": self.ctx_fs_stats.success_rate_percent,
                "total_agfs_calls": self.ctx_fs_stats.total_agfs_calls,
                "avg_agfs_calls_per_operation": self.ctx_fs_stats.avg_agfs_calls_per_operation,
                "agfs_total_latency_ms": self.ctx_fs_stats.agfs_total_latency_ms,
                "agfs_avg_latency_ms": self.ctx_fs_stats.agfs_avg_latency_ms,
                "agfs_success_count": self.ctx_fs_stats.agfs_success_count,
                "agfs_error_count": self.ctx_fs_stats.agfs_error_count,
                "agfs_success_rate_percent": self.ctx_fs_stats.agfs_success_rate_percent,
            }
...
def analyze_records(
    ...
    for record in records:
        ...
        if record.io_type == IOType.FS.value:
            ...
            if hasattr(record, "agfs_calls") and record.agfs_calls:
                ctx_fs_stats.total_operations += 1
                if record.success:
                    ctx_fs_stats.success_count += 1
                else:
                    ctx_fs_stats.error_count += 1

                ctx_fs_stats.total_agfs_calls += len(record.agfs_calls)

                for call in record.agfs_calls:
                    if isinstance(call, dict):
                        ctx_fs_stats.agfs_total_latency_ms += call.get("latency_ms", 0.0)
                        if call.get("success", True):
                            ctx_fs_stats.agfs_success_count += 1
                        else:
                            ctx_fs_stats.agfs_error_count += 1
                    else:
                        ctx_fs_stats.agfs_total_latency_ms += call.latency_ms
                        if call.success:
                            ctx_fs_stats.agfs_success_count += 1
                        else:
                            ctx_fs_stats.agfs_error_count += 1
    ...
    if ctx_fs_stats.total_operations > 0:
        ctx_fs_stats.success_rate_percent = (
            ctx_fs_stats.success_count / ctx_fs_stats.total_operations * 100
        )
        ctx_fs_stats.avg_agfs_calls_per_operation = (
            ctx_fs_stats.total_agfs_calls / ctx_fs_stats.total_operations
        )

        agfs_total = ctx_fs_stats.agfs_success_count + ctx_fs_stats.agfs_error_count
        if agfs_total > 0:
            ctx_fs_stats.agfs_avg_latency_ms = ctx_fs_stats.agfs_total_latency_ms / agfs_total
            ctx_fs_stats.agfs_success_rate_percent = (
                ctx_fs_stats.agfs_success_count / agfs_total * 100
            )

        stats.ctx_fs_stats = ctx_fs_stats
...
def print_analysis_stats(stats: RecordAnalysisStats) -> None:
    ...
    if stats.ctx_fs_stats:
        vfs = stats.ctx_fs_stats
        print("\n" + "=" * 80)
