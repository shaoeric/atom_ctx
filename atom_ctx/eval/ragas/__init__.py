...
"""
RAGAS evaluator integration for AtomCtx.
"""
...
def _create_ragas_llm_from_config() -> Optional[Any]:
    """
    Create a RAGAS-compatible LLM from AtomCtx VLM configuration or environment variables.

    Priority:
        1. Environment variables (RAGAS_LLM_API_KEY, RAGAS_LLM_API_BASE, RAGAS_LLM_MODEL)
        2. AtomCtx VLM configuration (~/.ctx/ctx.conf)
    ...
    try:
        ...
        logger.debug("AtomCtx config file not found, skipping VLM config")
        return None
    ...
    if not vlm_config.is_available():
        logger.warning(
            ...
            "Please configure VLM in ~/.ctx/ctx.conf or set environment variables "
    ...
    def __init__(
        ...
        Args:
            ...
            llm: LLM to use for evaluation (RAGAS LLM instance).
                 If None, uses AtomCtx VLM configuration.
    ...
    async def evaluate_dataset(self, dataset: EvalDataset) -> SummaryResult:
        ...
        if self.llm is None:
            raise ValueError(
                ...
                "  1. Environment variables: RAGAS_LLM_API_KEY, RAGAS_LLM_API_BASE, RAGAS_LLM_MODEL\n"
                "  2. AtomCtx VLM config in ~/.ctx/ctx.conf\n"
