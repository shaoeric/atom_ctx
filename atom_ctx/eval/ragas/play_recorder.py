...
Usage:
    uv run play_recorder.py --record_file ./records/io_recorder_20260214.jsonl --config_file ./ctx.conf
    uv run play_recorder.py --record_file ./records/io_recorder_20260214.jsonl --stats-only
    uv run play_recorder.py --record_file ./records/io_recorder_20260214.jsonl --config_file ./ctx.conf --fs
    uv run play_recorder.py --record_file ./records/io_recorder_20260214.jsonl --config_file ./ctx.conf --vikingdb
"""
...
def print_playback_stats(stats: PlaybackStats) -> None:
    ...
    if stats.total_ctx_fs_operations > 0:
        stats_dict = stats.to_dict()
        ctx_fs_stats = stats_dict.get("ctx_fs_stats", {})
        agfs_fs_stats = stats_dict.get("agfs_fs_stats", {})

        print("\nVikingFS Detailed Stats:")
        print(f"  Total VikingFS Operations: {ctx_fs_stats.get('total_operations', 0)}")
        print(f"  VikingFS Success Rate: {ctx_fs_stats.get('success_rate_percent', 0):.1f}%")
        print(
            f"  Average AGFS Calls per VikingFS Operation: {ctx_fs_stats.get('avg_agfs_calls_per_operation', 0):.2f}"
        )
...
Examples:
  ...
  uv run play_recorder.py --record_file ./records/io_recorder_20260214.jsonl --config_file ./ctx.conf --io-type fs --operation read

  # Save results to file
  uv run play_recorder.py --record_file ./records/io_recorder_20260214.jsonl --config_file ./ctx.conf --output results.json
        """,
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to AtomCtx config file (ctx.conf)",
    )
    parser.add_argument(
        "--stats-only",
        action="store_true",
