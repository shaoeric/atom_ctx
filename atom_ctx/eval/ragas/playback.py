...
class PlaybackStats:
    ...
    Attributes:
        ...
        ctx_fs_success_count: VikingFS operation success count
        ctx_fs_error_count: VikingFS operation error count
        ...
        total_ctx_fs_operations: Total number of VikingFS operations with AGFS calls
    """
    ...
    ctx_fs_success_count: int = 0
    ctx_fs_error_count: int = 0
    agfs_fs_success_count: int = 0
    agfs_fs_error_count: int = 0
    total_agfs_calls: int = 0
    total_ctx_fs_operations: int = 0
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        ctx_fs_success_rate = (
            self.ctx_fs_success_count / self.total_ctx_fs_operations * 100
            if self.total_ctx_fs_operations > 0
            else 0
        )
        ...
        avg_agfs_calls = (
            self.total_agfs_calls / self.total_ctx_fs_operations
            if self.total_ctx_fs_operations > 0
            else 0
        )
        ...
        return {
            ...
            "ctx_fs_stats": {
                "success_count": self.ctx_fs_success_count,
                "error_count": self.ctx_fs_error_count,
                "success_rate_percent": ctx_fs_success_rate,
                "total_operations": self.total_ctx_fs_operations,
...
class IOPlayback:
    ...
    Usage:
        playback = IOPlayback(config_file="./ctx.conf")
    ...
    def __init__(
        ...
        Args:
            config_file: Path to AtomCtx config file (ctx.conf)
    ...
    def _path_to_uri(self, path: str) -> str:
        ...
        return self._ctx_fs._path_to_uri(path)
    ...
    def _init_backends(self) -> None:
        ...
        if self.config_file:
            ...
            os.environ["CTX_CONFIG_FILE"] = self.config_file
        ...
        if self.enable_fs:
            # Use init_ctx_fs which handles mode (HTTP/Binding) automatically based on agfs_config
    ...
    async def _play_fs_operation(self, record: IORecord) -> PlaybackResult:
        ...
        try:
            ...
            if self.check_agfs_calls and hasattr(record, "agfs_calls") and record.agfs_calls:
                collector = _AGFSCallCollector(self._ctx_fs.agfs)
                original_agfs = self._ctx_fs.agfs
                self._ctx_fs.agfs = collector
        ...
        for i, record in enumerate(records):
            ...
            if record.io_type == IOType.FS.value:
                ...
                if hasattr(record, "agfs_calls") and record.agfs_calls:
                    stats.total_ctx_fs_operations += 1
                    if result.playback_success:
                        stats.ctx_fs_success_count += 1
                    else:
                        stats.ctx_fs_error_count += 1
