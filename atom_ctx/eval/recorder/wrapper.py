...
from atom_ctx.eval.recorder import (
    AGFSCallRecord,
    IORecorder,
    get_recorder,
)
...
class RecordingVikingFS:
    ...
    Usage:
        ...
        from atom_ctx.eval.recorder.wrapper import RecordingVikingFS

        init_recorder(enabled=True)
        fs = RecordingVikingFS(ctx_fs)
        await fs.read(uri)  # This will be recorded
    """

    def __init__(self, ctx_fs: Any, recorder: Optional[IORecorder] = None):
        """
        Initialize wrapper.

        Args:
            ctx_fs: VikingFS instance to wrap
            recorder: IORecorder instance (uses global if None)
        """
        self._fs = ctx_fs
        self._recorder = recorder or get_recorder()
        self._original_agfs = getattr(ctx_fs, "agfs", None)
...
class RecordingVikingDB:
    ...
    Usage:
        from atom_ctx.eval.recorder import init_recorder
        from atom_ctx.eval.recorder.wrapper import RecordingVikingDB
    ...
    def __init__(self, vector_db: Any, recorder: Optional[IORecorder] = None):
        """
        Initialize wrapper.

        Args:
            vector_db: Vector store instance to wrap
            recorder: IORecorder instance (uses global if None)
        """
        self._db = vector_db
        self._recorder = recorder or get_recorder()
