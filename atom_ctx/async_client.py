...
"""
Async AtomCtx client implementation (embedded mode only).
...
from atom_ctx.client import LocalClient, Session
from atom_ctx.service.debug_service import SystemStatus
from atom_ctx.telemetry import TelemetryRequest
...
class AsyncAtomCtx:
    """
    AtomCtx main client class (Asynchronous, embedded mode only).

    Uses local storage and auto-starts services (singleton).
    For HTTP mode, use AsyncHTTPClient or SyncHTTPClient instead.

    Examples:
        client = AsyncAtomCtx(path="./data")
        await client.initialize()
    """

    _instance: Optional["AsyncAtomCtx"] = None
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize AtomCtx client (embedded mode).
    ...
    async def add_resource(
        ...
        **kwargs,
        ...
        """
        Add a resource (file/URL) to AtomCtx.
    ...
    async def glob(self, pattern: str, uri: str = "ctx://") -> Dict:
    ...
    async def export_ctxpack(self, uri: str, to: str) -> str:
        ...
        """
        await self._ensure_initialized()
        return await self._client.export_ctxpack(uri, to)
    ...
    async def import_ctxpack(
        ...
        """
        await self._ensure_initialized()
        return await self._client.import_ctxpack(file_path, parent, force=force, vectorize=vectorize)
