...
from atom_ctx_cli.exceptions import ConflictError, NotFoundError
...
class WatchManager:
    ...
    STORAGE_URI = "ctx://resources/.watch_tasks.json"
    STORAGE_BAK_URI = "ctx://resources/.watch_tasks.json.bak"
    STORAGE_TMP_URI = "ctx://resources/.watch_tasks.json.tmp"

    def __init__(self, ctx_fs: Optional[Any] = None):
        """Initialize WatchManager.

        Args:
            ctx_fs: VikingFS instance for persistence storage
        """
    ...
    async def _load_tasks(self) -> None:
        ...
        try:
            ...
            data = None
            try:
                content = await self._ctx_fs.read_file(self.STORAGE_URI, ctx=ctx)
            ...
            if data is None:
                try:
                    bak_content = await self._ctx_fs.read_file(self.STORAGE_BAK_URI, ctx=ctx)
    ...
    async def _save_tasks(self) -> None:
        ...
        try:
            ...
            supports_atomic = all(
                hasattr(self._ctx_fs, name) for name in ("mv", "rm", "exists", "write_file")
            )
            if not supports_atomic:
                await self._ctx_fs.write_file(self.STORAGE_URI, content, ctx=ctx)
                logger.debug(f"[WatchManager] Saved {len(self._tasks)} tasks to storage")
                return

            await self._ctx_fs.write_file(self.STORAGE_TMP_URI, content, ctx=ctx)

            try:
                if await self._ctx_fs.exists(self.STORAGE_BAK_URI, ctx=ctx):
                    await self._ctx_fs.rm(self.STORAGE_BAK_URI, ctx=ctx)
            except Exception:
                pass

            try:
                if await self._ctx_fs.exists(self.STORAGE_URI, ctx=ctx):
                    await self._ctx_fs.mv(self.STORAGE_URI, self.STORAGE_BAK_URI, ctx=ctx)
            except Exception as e:
                logger.warning(f"[WatchManager] Failed to rotate task storage backup: {e}")

            await self._ctx_fs.mv(self.STORAGE_TMP_URI, self.STORAGE_URI, ctx=ctx)
            logger.debug(f"[WatchManager] Saved {len(self._tasks)} tasks to storage")
        except Exception as e:
