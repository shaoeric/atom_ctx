...
"""
Hierarchical retriever for AtomCtx.
...
from atom_ctx.retrieve.memory_lifecycle import hotness_score
from atom_ctx.retrieve.retrieval_stats import get_stats_collector
from atom_ctx.server.identity import RequestContext, Role
from atom_ctx.storage import VikingDBManager, VikingDBManagerProxy
...
from atom_ctx_cli.utils.rerank import RerankClient
...
class HierarchicalRetriever:
    ...
    async def _convert_to_matched_contexts(
        ...
        for c in candidates:
            ...
            relations = []
            if get_ctx_fs():
                related_uris = await get_ctx_fs().get_relations(c.get("uri", ""), ctx=ctx)
                if related_uris:
                    related_abstracts = await get_ctx_fs().read_batch(
    ...
    def _get_root_uris_for_type(
        ...
        if context_type is None:
            return [
                f"ctx://user/{user_space}/memories",
                f"ctx://agent/{agent_space}/memories",
                "ctx://resources",
                f"ctx://agent/{agent_space}/skills",
            ]
            ...
            return [f"ctx://agent/{agent_space}/skills"]
        return []
