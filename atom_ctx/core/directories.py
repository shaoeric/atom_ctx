...
"""
Preset directory structure definitions for AtomCtx.

AtomCtx uses a virtual filesystem where all directories are data records.
            ...
            DirectoryDefinition(
                ...
                children=[
                    ...
                    DirectoryDefinition(
                        ...
                        "Examples: AtomCtx project, colleague Zhang San, certain technical concept. "
...
def get_context_type_for_uri(uri: str) -> str:
    ...
    elif uri.startswith("ctx://session"):
        return ContextType.MEMORY.value
    return ContextType.RESOURCE.value
...
class DirectoryInitializer:
    ...
    async def initialize_user_directories(self, ctx: RequestContext) -> int:
        ...
        if "user" not in PRESET_DIRECTORIES:
            return 0
        user_space_root = f"ctx://user/{ctx.user.user_space_name()}"
    ...
    async def initialize_agent_directories(self, ctx: RequestContext) -> int:
        ...
        if "agent" not in PRESET_DIRECTORIES:
            return 0
        agent_space_root = f"ctx://agent/{ctx.user.agent_space_name()}"
        agent_tree = PRESET_DIRECTORIES["agent"]
        created = await self._ensure_directory(
            uri=agent_space_root,
            parent_uri="ctx://agent",
            defn=agent_tree,
            scope="agent",
            ctx=ctx,
        )
    ...
    async def _check_agfs_files_exist(self, uri: str, ctx: RequestContext) -> bool:
        ...
        try:
            ctx_fs = get_ctx_fs()
            await ctx_fs.abstract(uri, ctx=ctx)
            return True
        except Exception:
            return False
    ...
    async def _create_agfs_structure(
        ...
        await get_ctx_fs().write_context(
            uri=uri,
            abstract=abstract,
            overview=overview,
