...
"""Unified context class for AtomCtx."""
...
from atom_ctx.utils.time_utils import format_iso8601, parse_iso_datetime
...
class Context:
    """
    Unified context class for all context types in AtomCtx.
    """
    ...
    def _derive_owner_space(self, user: Optional[UserIdentifier]) -> str:
        ...
        if not user:
            return ""
        if self.uri.startswith("ctx://agent/"):
            return user.agent_space_name()
        if self.uri.startswith("ctx://user/") or self.uri.startswith("ctx://session/"):
