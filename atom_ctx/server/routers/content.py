...
"""Content endpoints for AtomCtx HTTP Server."""
...
from atom_ctx.server.models import ErrorInfo, Response
...
async def read(
    ...
    if text:
        from atom_ctx.session.memory.utils.content import deserialize_content
...
async def reindex(
    ...
    # Validate URI exists
    if not await ctx_fs.exists(uri, ctx=_ctx):
        return Response(
            status="error",
...
async def _do_reindex(
    ...
    path = ctx_fs._uri_to_path(uri, ctx=ctx)
