...
"""System endpoints for AtomCtx HTTP Server."""
...
async def health_check(request: Request):
    ...
    try:
        ...
        x_atom_ctx_user = request.headers.get("X-AtomCtx-User")

        # Check if we have auth or in dev mode
        api_key_manager = getattr(request.app.state, "api_key_manager", None)
        if api_key_manager is None:
            # Dev mode - use default user
            result["user_id"] = x_atom_ctx_user or "default"
            ...
            try:
                identity = await resolve_identity(
                    ...
                    x_atom_ctx_account=request.headers.get("X-AtomCtx-Account"),
                    x_atom_ctx_user=x_atom_ctx_user,
                    x_atom_ctx_agent=request.headers.get("X-AtomCtx-Agent"),
                )
...
async def readiness_check(request: Request):
    ...
    try:
        ctx_fs = get_ctx_fs()
        storage = ctx_fs._get_vector_store()
        if storage:
