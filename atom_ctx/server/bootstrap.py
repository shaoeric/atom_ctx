...
"""Bootstrap script for AtomCtx HTTP Server."""
...
from atom_ctx_cli.utils.logger import configure_uvicorn_logging
...
def _get_version() -> str:
    try:
        from atom_ctx import __version__
...
def main():
    """Main entry point for ctx-server command."""
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"ctx-server {_get_version()}",
    )
    parser.add_argument(
        "--host",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to ctx.conf config file",
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=None,
        help="Number of uvicorn worker processes (default: 1, or server.workers in ctx.conf)",
    )
    parser.add_argument(
        "--bot",
        action="store_true",
    ...
    parser.add_argument(
        "--bot-log-dir",
        type=str,
        default=os.path.expanduser("~/.ctx/data/bot/logs"),
        help="Directory to store vikingbot log files",
    )

    args = parser.parse_args()

    # Set CTX_CONFIG_FILE environment variable if --config is provided
    # This allows AtomCtxConfigSingleton to load from the specified config file
    if args.config is not None:
        os.environ["CTX_CONFIG_FILE"] = args.config

    from atom_ctx_cli.utils.config.ctx_config import AtomCtxConfigSingleton

    # Load server config from ctx.conf
    try:
        config = load_server_config(args.config)
        AtomCtxConfigSingleton.initialize(config_path=args.config)
    ...
    print(f"AtomCtx HTTP Server is running on {config.host}:{config.port}{workers_info}")
    if config.with_bot:
    ...
    try:
        ...
        if workers > 1:
            ...
            # pick it up (ServerConfig already reads CTX_CONFIG_FILE).
            uvicorn.run(
                "atom_ctx.server.app:create_app",
                factory=True,
                host=config.host,
                port=config.port,
                workers=workers,
