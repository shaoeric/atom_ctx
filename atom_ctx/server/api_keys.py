...
"""API Key management for AtomCtx multi-tenant HTTP Server."""
...
class APIKeyManager:
    ...
    def __init__(self, root_key: str, ctx_fs: VikingFS, encryption_enabled: bool = False):
        """Initialize APIKeyManager.

        Args:
            root_key: Global root API key for administrative access.
            ctx_fs: VikingFS client for persistent storage of user keys.
    ...
    async def _read_json(self, path: str) -> Optional[dict]:
        ...
        try:
            ...
            content = self._ctx_fs.agfs.read(path)
            ...
            raw = await self._ctx_fs.decrypt_bytes(account_id, raw)
    ...
    async def _write_json(self, path: str, data: dict) -> None:
        ...
        parts = path.split("/")
        account_id = parts[2] if len(parts) >= 3 else "default"
        content = await self._ctx_fs.encrypt_bytes(account_id, content)

        # Ensure parent directories exist
        self._ensure_parent_dirs(path)
        # Write file directly using AGFS
        self._ctx_fs.agfs.write(path, content)
        ...
        if path.startswith("/local/"):
            ...
            if parts:
                ...
                for i in range(1, len(parts)):
                    parent = "/" + "/".join(parts[:i])
                    try:
                        self._ctx_fs.agfs.mkdir(parent)
                    except Exception:
                        pass
