...
"""Server configuration for AtomCtx HTTP Server."""
...
from atom_ctx_cli.utils.config.config_utils import format_validation_error
from atom_ctx_cli.utils.config.consts import (
    DEFAULT_CONFIG_DIR,
    DEFAULT_CTX_CONF,
    CTX_CONFIG_ENV,
    SYSTEM_CONFIG_DIR,
...
class ServerConfig(BaseModel):
    """Server configuration (from the ``server`` section of ctx.conf)."""
...
def load_server_config(config_path: Optional[str] = None) -> ServerConfig:
    """Load server configuration from ctx.conf.

    Reads the ``server`` section of ctx.conf and also ensures the full
    ctx.conf is loaded into the AtomCtxConfigSingleton so that model
    ...
    Args:
        config_path: Explicit path to ctx.conf.
...
def validate_server_config(config: ServerConfig) -> None:
    ...
    if not _is_localhost(config.host):
        ...
        logger.error(
            "To fix, either:\n"
            "  1. Set server.root_api_key in ctx.conf, or\n"
