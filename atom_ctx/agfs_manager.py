...
class AGFSManager:
    ...
    def start(self) -> None:
        ...
        self.ctxfs_path.mkdir(parents=True, exist_ok=True)
        (self.data_path / "_system" / "queue").mkdir(parents=True, exist_ok=True)
        # NOTICE: should use ctx://temp/ instead of self.ctxfs_path / "temp"
        # Create temp directory for Parser use
        # (self.ctxfs_path / "temp").mkdir(exist_ok=True)
