...
HKDF_SALT = b"atom_ctx-kek-salt-v1"
HKDF_INFO_PREFIX = b"atom_ctx:kek:v1:"
...
class VaultProvider(BaseProvider):
    ...
    def __init__(
        ...
        kv_version: int = 1,
        root_key_name: str = "atom_ctx-root-key",
        encrypted_root_key_key: str = "atom_ctx-encrypted-root-key",
    ):
        """
        ...
        Args:
            ...
            root_key_name: Transit engine key name (default: "atom_ctx-root-key")
            encrypted_root_key_key: KV engine key path (default: "atom_ctx-encrypted-root-key")
        """
        self.addr = addr
        self.token = token
...
class VolcengineKMSProvider(BaseProvider):
    ...
    ROOT_KEY_FILENAME = "atom_ctx-volcengine-root-key.enc"
    ...
    def __init__(
        ...
        Args:
            ...
            key_file: Path to encrypted root key file (default: ~/.ctx/atom_ctx-volcengine-root-key.enc)
        """
        self.region = region
        ...
        if key_file:
            ...
            self.key_file = Path.home() / ".ctx" / self.ROOT_KEY_FILENAME
...
def create_root_key_provider(
    ...
    if provider_type == "local":
        ...
        key_file_path = local_config.get("key_file", "~/.ctx/master.key")
        ...
        root_key_name = vault_config.get("root_key_name", "atom_ctx-root-key")
