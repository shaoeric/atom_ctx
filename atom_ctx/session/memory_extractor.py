...
from atom_ctx.core.context import Context, ContextType, Vectorize
...
class MemoryExtractor:
    ...
    async def extract(
        ...
        *,
        ...
        try:
            from atom_ctx_cli.utils.llm import parse_json_from_response
    ...
    async def create_memory(
        ...
        if candidate.category == MemoryCategory.PROFILE:
            payload = await self._append_to_profile(candidate, ctx_fs, ctx=ctx)
            if not payload:
                return None
            user_space = ctx.user.user_space_name()
            memory_uri = f"ctx://user/{user_space}/memories/profile.md"
            memory = Context(
                uri=memory_uri,
                parent_uri=f"ctx://user/{user_space}/memories",
                is_leaf=True,
        ...
        if candidate.category in [
            ...
            parent_uri = f"ctx://user/{ctx.user.user_space_name()}/{cat_dir}"
        else:  # CASES, PATTERNS
            parent_uri = f"ctx://agent/{ctx.user.agent_space_name()}/{cat_dir}"
        ...
        try:
            await ctx_fs.write_file(memory_uri, candidate.content, ctx=ctx)
    ...
    async def _append_to_profile(
        ...
        uri = f"ctx://user/{ctx.user.user_space_name()}/memories/profile.md"
        existing = ""
        try:
            existing = await ctx_fs.read_file(uri, ctx=ctx) or ""
        except Exception:
            pass

        if not existing.strip():
            await ctx_fs.write_file(uri=uri, content=candidate.content, ctx=ctx)
            ...
            if not payload:
                ...
                return None
            await ctx_fs.write_file(uri=uri, content=payload.content, ctx=ctx)
    ...
    async def _merge_tool_memory(
        ...
        uri = f"ctx://agent/{agent_space}/memories/tools/{tool_name}.md"
        ctx_fs = get_ctx_fs()
        ...
        if not existing.strip():
            ...
            await ctx_fs.write_file(uri=uri, content=merged_content, ctx=ctx)
    ...
    def _create_tool_context(
        ...
        return Context(
            uri=uri,
            parent_uri=f"ctx://agent/{agent_space}/memories/tools",
            is_leaf=True,
    ...
    async def _merge_skill_memory(
        ...
        agent_space = ctx.user.agent_space_name()
        uri = f"ctx://agent/{agent_space}/memories/skills/{skill_name}.md"
        ctx_fs = get_ctx_fs()
    ...
    def _create_skill_context(
        ...
        return Context(
            uri=uri,
            parent_uri=f"ctx://agent/{agent_space}/memories/skills",
            is_leaf=True,
            abstract=abstract_override or candidate.abstract,
            context_type=ContextType.MEMORY.value,
            category=candidate.category.value,
