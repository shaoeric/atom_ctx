...
"""
Session Compressor for AtomCtx.
...
from atom_ctx.core.context import Context, Vectorize
...
class SessionCompressor:
    ...
    async def _merge_into_existing(
        ...
        try:
            existing_content = await ctx_fs.read_file(target_memory.uri, ctx=ctx)
            ...
            await ctx_fs.write_file(target_memory.uri, payload.content, ctx=ctx)
    ...
    async def _delete_existing_memory(
        self, memory: Context, ctx_fs, ctx: RequestContext
    ) -> bool:
        """Hard delete an existing memory file and clean up its vector record."""
        try:
            await ctx_fs.rm(memory.uri, recursive=False, ctx=ctx)
        except Exception as e:
                        ...
                        for action in actions:
                            if action.decision == MemoryActionDecision.DELETE:
                                with telemetry.measure("memory.extract.stage.delete_existing"):
                                    deleted = ctx_fs and await self._delete_existing_memory(
                                        action.memory, ctx_fs, ctx=ctx
                                    )
                                if deleted:
                                    stats.deleted += 1
                                ...
                                if candidate.category in MERGE_SUPPORTED_CATEGORIES and ctx_fs:
                                    with telemetry.measure("memory.extract.stage.merge_existing"):
                                        merged = await self._merge_into_existing(
                                            candidate, action.memory, ctx_fs, ctx=ctx
                                        )
                                    if merged:
                                        stats.merged += 1
    ...
    async def _create_relations(
        ...
        try:
            ...
            for uri in resource_uris:
                if await self._uri_exists(uri, ctx_fs, ctx):
    ...
    async def _uri_exists(self, uri: str, ctx_fs, ctx: RequestContext) -> bool:
        """Check if a URI exists."""
        try:
            await ctx_fs.read_file(uri, ctx=ctx)
            return True
        except Exception:
            return False
