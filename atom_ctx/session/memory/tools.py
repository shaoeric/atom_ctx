...
from atom_ctx.session.memory.utils import parse_memory_file_with_fields
...
class MemoryReadTool(MemoryTool):
    ...
    def parameters(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                "uri": {
                    "type": "string",
                    "description": "Memory URI to read, e.g., 'ctx://user/user123/memories/profile.md'",
                },
            },
            "required": ["uri"],
        }
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            uri = kwargs.get("uri", "")
            content = await ctx_fs.read_file(
                uri,
                ctx=ctx.request_ctx,
            )
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # 多搜索 10 个，过滤抽象文件后再截断
            search_result = await ctx_fs.search(
                query,
    ...
    def parameters(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                "uri": {
                    "type": "string",
                    "description": "Directory URI to list, e.g., 'ctx://user/user123/memories'",
                },
            },
            "required": ["uri"],
        }
