...
from atom_ctx.session.memory.dataclass import MemoryOperations
...
from atom_ctx.storage.ctx_fs import VikingFS, get_ctx_fs
...
class ExtractLoop:
    ...
    def __init__(
        ...
        Args:
            vlm: VLM instance (from atom_ctx.models.vlm.base)
            ctx_fs: VikingFS instance for storage operations
        ...
        """
        self.vlm = vlm
        self.ctx_fs = ctx_fs or get_ctx_fs()
...
```
    ...
    async def _execute_tool(
        ...
        # 创建 ToolContext
        from atom_ctx.server.identity import ToolContext
        ...
        try:
            result = await tool.execute(self.ctx_fs, tool_ctx, **tool_call.arguments)
            return result
        except Exception as e:
    ...
    async def _check_unread_existing_files(
        ...
        from atom_ctx.session.memory.utils.uri import resolve_flat_model_uri
        ...
        for field_name in memory_type_fields:
            ...
            for item in items:
                ...
                if uri in self._read_files:
                    continue
                try:
                    await self.ctx_fs.read_file(uri, ctx=self.ctx)
    ...
    async def _add_refetch_results_to_messages(
        ...
        for uri in refetch_uris:
            try:
                content = await self.ctx_fs.read_file(uri, ctx=self.ctx)
