...
from atom_ctx.session.memory.dataclass import MemoryField
from atom_ctx.session.memory.memory_type_registry import MemoryTypeRegistry
from atom_ctx.session.memory.merge_op import MergeOpFactory, PatchOp
from atom_ctx.session.memory.merge_op.base import FieldType, SearchReplaceBlock, StrPatch
...
from atom_ctx.session.memory.utils.uri import ResolvedOperation
...
class MemoryUpdater:
    ...
    def _get_ctx_fs(self):
        """Get or create VikingFS instance."""
        if self._ctx_fs is None:
            self._ctx_fs = get_ctx_fs()
        return self._ctx_fs
    ...
    async def _apply_write(self, flat_model: Any, uri: str, ctx: RequestContext, extract_context: Any = None, memory_type: str = None) -> None:
        ...
        await ctx_fs.write_file(uri, full_content, ctx=ctx)
    ...
    def _is_patch_format(self, content: Any) -> bool:
        ...
        from atom_ctx.session.memory.merge_op.patch import StrPatch
    ...
    async def _apply_edit(self, flat_model: Any, uri: str, ctx: RequestContext) -> None:
        ...
        # Read current memory
        try:
            current_full_content = await ctx_fs.read_file(uri, ctx=ctx) or ""
        ...
        if "content" in model_dict and "content" not in field_schema_map:
            from atom_ctx.session.memory.merge_op import PatchOp
        ...
        await ctx_fs.write_file(uri, new_full_content, ctx=ctx)
    ...
    async def _apply_delete(self, uri: str, ctx: RequestContext) -> None:
        ...
        try:
            await ctx_fs.rm(uri, recursive=False, ctx=ctx)
    ...
    async def _apply_edit_overview(self, overview_model: Any, uri: str, ctx: RequestContext) -> None:
        ...
        current_overview = ""
        try:
            current_overview = await ctx_fs.read_file(uri, ctx=ctx) or ""
        except NotFoundError:
        ...
        # Write new overview
        await ctx_fs.write_file(uri, new_overview, ctx=ctx)
    ...
    async def _write_abstract_from_overview(
        ...
        try:
            await ctx_fs.write_file(abstract_uri, abstract, ctx=ctx)
    ...
    async def _vectorize_memories(
        ...
        for uri in uris_to_vectorize:
            try:
                ...
                content = await ctx_fs.read_file(uri, ctx=ctx) or ""
                ...
                from atom_ctx.core.context import Context, ContextLevel, Vectorize
