...
from atom_ctx.session.memory.core import ExtractContextProvider
...
class SessionExtractContextProvider(ExtractContextProvider):
    ...
    def _detect_language(self) -> str:
        """检测输出语言"""
        from atom_ctx.session.memory.utils import detect_language_from_conversation
...
After exploring, analyze the conversation and output ALL memory write/edit/delete operations in a single response. Do not output operations one at a time - gather all changes first, then return them together."""
    ...
    async def prefetch(
        ...
        for overview_uri in overview_files:
            try:
                result_str = await read_tool.execute(ctx_fs, tool_ctx, uri=overview_uri)
                add_tool_call_pair_to_messages(
        ...
        if search_tool and ctx_fs and ls_dirs:
        ...
        for file_uri in read_files:
            try:
                result_str = await read_tool.execute(ctx_fs, tool_ctx, uri=file_uri)
                add_tool_call_pair_to_messages(
                    messages=pre_fetch_messages,
                    call_id=call_id_seq,
                    tool_name='read',
