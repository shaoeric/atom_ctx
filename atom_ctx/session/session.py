...
"""Session management for AtomCtx.
...
from atom_ctx.message import Message, Part
...
from atom_ctx_cli.utils import get_logger, run_async
...
class Session:
    ...
    def __init__(
        ...
        self._session_uri = f"ctx://session/{self.user.user_space_name()}/{self.session_id}"
    ...
    async def load(self):
        ...
        try:
            content = await self._ctx_fs.read_file(
        ...
        try:
            history_items = await self._ctx_fs.ls(f"{self._session_uri}/history", ctx=self.ctx)
            archives = [
        ...
        # Load .meta.json
        try:
            meta_content = await self._ctx_fs.read_file(
    ...
    async def exists(self) -> bool:
        ...
        try:
            await self._ctx_fs.stat(self._session_uri, ctx=self.ctx)
            return True
        except Exception:
            return False
    ...
    async def ensure_exists(self) -> None:
        ...
        if await self.exists():
            return
        await self._ctx_fs.mkdir(self._session_uri, exist_ok=True, ctx=self.ctx)
        await self._ctx_fs.write_file(f"{self._session_uri}/messages.jsonl", "", ctx=self.ctx)
    ...
    async def commit_async(self) -> Dict[str, Any]:
        ...
        session_path = self._ctx_fs._uri_to_path(self._session_uri, ctx=self.ctx)
    ...
    async def _run_memory_extraction(
        ...
        from atom_ctx.telemetry import OperationTelemetry, bind_telemetry
            ...
            with bind_telemetry(telemetry):
                ...
                if self._ctx_fs:
                    for usage in usage_records:
                        try:
                            await self._ctx_fs.link(self._session_uri, usage.uri, ctx=self.ctx)
    ...
    async def _list_archive_refs(self) -> List[Dict[str, Any]]:
        ...
        if not self._ctx_fs or self.compression.compression_index <= 0:
            return []
    ...
    async def _get_completed_archive_refs(
        ...
        for archive in await self._list_archive_refs():
            if exclude and archive["archive_uri"] == exclude:
                continue
            try:
                await self._ctx_fs.read_file(f"{archive['archive_uri']}/.done", ctx=self.ctx)
            except Exception:
                continue
    ...
    async def _read_archive_overview(self, archive_uri: str) -> str:
        ...
        try:
            overview = await self._ctx_fs.read_file(f"{archive_uri}/.overview.md", ctx=self.ctx)
        except Exception:
            return ""
        return overview or ""
    ...
    async def _read_archive_abstract(self, archive_uri: str, overview: str = "") -> str:
        ...
        try:
            abstract = await self._ctx_fs.read_file(f"{archive_uri}/.abstract.md", ctx=self.ctx)
        except Exception:
            abstract = ""
    ...
    async def _read_archive_messages(self, archive_uri: str) -> List[Message]:
        ...
        try:
            content = await self._ctx_fs.read_file(f"{archive_uri}/messages.jsonl", ctx=self.ctx)
        except Exception:
            return []
    ...
    async def _wait_for_previous_archive_done(self, archive_index: int) -> bool:
        ...
        if archive_index <= 1 or not self._ctx_fs:
            return True

        previous_archive_uri = f"{self._session_uri}/history/archive_{archive_index - 1:03d}"
        while True:
            try:
                await self._ctx_fs.read_file(f"{previous_archive_uri}/.done", ctx=self.ctx)
                return True
            except Exception:
                pass
    ...
    def _write_archive(
        ...
        run_async(
            ctx_fs.write_file(uri=f"{archive_uri}/.abstract.md", content=abstract, ctx=self.ctx)
        )
        run_async(
            ctx_fs.write_file(uri=f"{archive_uri}/.overview.md", content=overview, ctx=self.ctx)
        )
    ...
    def _write_relations(self) -> None:
        ...
        for usage in self._usage_records:
            try:
                run_async(ctx_fs.link(self._session_uri, usage.uri, ctx=self.ctx))
    ...
    async def _write_relations_async(self) -> None:
        ...
        for usage in self._usage_records:
            try:
                await ctx_fs.link(self._session_uri, usage.uri, ctx=self.ctx)
                logger.debug(f"Created relation: {self._session_uri} -> {usage.uri}")
            except Exception as e:
                logger.warning(f"Failed to create relation to {usage.uri}: {e}")
