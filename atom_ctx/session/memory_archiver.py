...
from atom_ctx.storage.expr import And, Eq
...
class MemoryArchiver:
    ...
    Moves them to ``{scope}/_archive/`` using ``ctx_fs.mv()`` so
    ...
    def __init__(
        ...
        Args:
            ctx_fs: VikingFS instance for filesystem operations.
    ...
    async def scan(
        ...
        Args:
            scope_uri: Root URI to scan (e.g. ``ctx://memories/``).
    ...
    async def archive(
        ...
        ``ctx_fs.mv()``, which atomically updates the vector index.
        ...
        for candidate in candidates:
            ...
            try:
                await self.ctx_fs.mv(candidate.uri, archive_uri, ctx=ctx)
                result.archived += 1
                logger.info(
    ...
    async def restore(
        ...
        try:
            await self.ctx_fs.mv(archived_uri, original_uri, ctx=ctx)
