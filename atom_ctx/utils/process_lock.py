...
Prevents multiple AtomCtx processes from contending for the same data
...
LOCK_FILENAME = ".ctx.pid"


class DataDirectoryLocked(RuntimeError):
    """Raised when another AtomCtx process holds the data directory lock."""
...
def acquire_data_dir_lock(data_dir: str) -> str:
    ...
    if existing_pid and existing_pid != my_pid and _is_pid_alive(existing_pid):
        raise DataDirectoryLocked(
            f"Another AtomCtx process (PID {existing_pid}) is already using "
            f"the data directory '{data_dir}'. Running multiple AtomCtx "
            ...
            f"  1. Use HTTP mode: start a single ctx-server and connect "
