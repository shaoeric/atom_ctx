...
from atom_ctx.core.mcp_converter import is_mcp_format, mcp_to_skill
from atom_ctx.core.skill_loader import SkillLoader
from atom_ctx.server.identity import RequestContext
from atom_ctx.server.local_input_guard import deny_direct_local_skill_input
...
from atom_ctx.utils.zip_safe import safe_extract_zip
...
class SkillProcessor:
    ...
    async def process_skill(
        ...
        Args:
            ...
            ctx_fs: VikingFS instance for storage
        ...
        context = Context(
            uri=f"ctx://agent/skills/{skill_dict['name']}",
            parent_uri="ctx://agent/skills",
            is_leaf=False,
        ...
        skill_dir_uri = f"ctx://agent/skills/{context.meta['name']}"
    ...
    async def _write_auxiliary_files(
        ...
        for aux_file in auxiliary_files:
            ...
            if is_text:
                await ctx_fs.write_file(aux_uri, file_bytes.decode("utf-8"), ctx=ctx)
            else:
                await ctx_fs.write_file_bytes(aux_uri, file_bytes, ctx=ctx)
    ...
    async def _index_skill(self, context: Context, skill_dir_uri: str):
        ...
        context.parent_uri = "ctx://agent/skills"
