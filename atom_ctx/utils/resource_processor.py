...
"""
Context Processor for AtomCtx.

Handles coordinated writes and self-iteration processes
as described in the AtomCtx design document.
"""
...
from atom_ctx.utils.embedding_utils import index_resource
from atom_ctx.utils.summarizer import Summarizer
from atom_ctx_cli.utils import get_logger
from atom_ctx_cli.utils.storage import StoragePath
    ...
    async def process_resource(
        ...
        **kwargs,
        ...
        with telemetry.measure("resource.process"):
            # ============ Phase 1: Parse source and writes to temp ctx fs ============
            try:
                ...
                with ctx_fs.bind_request_context(ctx):
            ...
            try:
                ...
                with get_ctx_fs().bind_request_context(ctx):
                ...
                try:
                    if parse_result.temp_dir_path:
                        await get_ctx_fs().delete_temp(parse_result.temp_dir_path, ctx=ctx)
                except Exception:
                    pass
            ...
            if root_uri and temp_uri:
                ...
                target_exists = await ctx_fs.exists(root_uri, ctx=ctx)

                if not target_exists:
                    # 第一次添加：锁保护下将 temp 移到 final
                    dst_path = ctx_fs._uri_to_path(root_uri, ctx=ctx)
                    parent_path = dst_path.rsplit("/", 1)[0] if "/" in dst_path else dst_path

                    parent_uri = "/".join(root_uri.rstrip("/").rsplit("/", 1)[:-1])
                    if parent_uri:
                        await ctx_fs.mkdir(parent_uri, exist_ok=True, ctx=ctx)
                    ...
                    async with LockContext(lock_manager, [parent_path], lock_mode="point"):
                        ...
                        src_path = ctx_fs._uri_to_path(temp_uri, ctx=ctx)
                        await asyncio.to_thread(ctx_fs.agfs.mv, src_path, dst_path)
                    ...
                    try:
                        await ctx_fs.delete_temp(parse_result.temp_dir_path, ctx=ctx)
                    except Exception:
                        pass
                    ...
                    resource_path = ctx_fs._uri_to_path(root_uri, ctx=ctx)
                    lifecycle_lock_handle_id = await self._try_acquire_lifecycle_lock(
