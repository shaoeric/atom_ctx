...
"""
Embedding utilities for AtomCtx.
...
from atom_ctx.core.context import Context, ContextLevel, ResourceContentType, Vectorize
...
from atom_ctx_cli.utils import CtxURI, get_logger
...
async def vectorize_file(
    ...
    try:
        ...
        embedding_cfg = get_atom_ctx_config().embedding
        ...
        if content_type is None:
            ...
            if summary and effective_text_source in {"summary_first", "summary_only"}:
                ...
                try:
                    content = await ctx_fs.read_file(file_path, ctx=ctx)
...
async def index_resource(
    ...
    if await ctx_fs.exists(abstract_uri, ctx=ctx):
        content = await ctx_fs.read_file(abstract_uri, ctx=ctx)
        if isinstance(content, bytes):
            abstract = content.decode("utf-8")

    if await ctx_fs.exists(overview_uri, ctx=ctx):
        content = await ctx_fs.read_file(overview_uri, ctx=ctx)
        if isinstance(content, bytes):
    ...
    # 2. Index Files
    try:
        files = await ctx_fs.ls(uri, ctx=ctx)
