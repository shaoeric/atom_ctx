...
from atom_ctx.parse import DocumentConverter, parse
...
class UnifiedResourceProcessor:
    ...
    async def _process_url(self, url: str, instruction: str, **kwargs) -> ParseResult:
        """Process URL source."""
        from atom_ctx.utils.code_hosting_utils import is_git_repo_url, validate_git_ssh_uri
        ...
        if self._is_feishu_url(url):
            ...
            if parser is None:
                raise ImportError(
                    ...
                    "Install lark-oapi: pip install 'atom-ctx[bot-feishu]'"
                )
