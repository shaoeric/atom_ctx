...
"""
Tree Builder for AtomCtx.

Converts parsed document trees into AtomCtx context objects with proper
...
from atom_ctx.parse.parsers.media.utils import get_media_base_uri, get_media_type
from atom_ctx.server.identity import RequestContext
from atom_ctx.storage.ctx_fs import get_ctx_fs
from atom_ctx.utils import parse_code_hosting_url
...
class TreeBuilder:
    """
    Builds AtomCtx context tree from parsed documents (v5.0).
    ...
    async def finalize_from_temp(
        ...
        entries = await ctx_fs.ls(temp_uri, ctx=ctx)
        ...
        doc_name = CtxURI.sanitize_segment(original_name)
        ...
        if to_uri:
            ...
            if parent_uri:
                ...
                try:
                    stat_result = await ctx_fs.stat(parent_uri, ctx=ctx)
            ...
            candidate_uri = CtxURI(base_uri).join(final_doc_name).uri
