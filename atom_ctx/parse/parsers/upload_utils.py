...
async def upload_text_files(
    ...
    for file_path, rel_path in file_paths:
        try:
            ...
            target_uri = f"{ctx_uri_base}/{safe_rel}"
            content = file_path.read_bytes()
            content = detect_and_convert_encoding(content, file_path)
            await ctx_fs.write_file_bytes(target_uri, content)
            uploaded_count += 1
        except Exception as exc:
...
async def upload_directory(
    ...
    parent_uris: Set[str] = {ctx_uri_base}
    ...
    def _create_all_dirs() -> None:
        for dir_uri in sorted(parent_uris):
            _mkdir_with_parents(ctx_fs._uri_to_path(dir_uri))
    ...
    async def _upload_one(idx: int, file_path: Path, target_uri: str) -> None:
        async with sem:
            ...
            def _do() -> None:
                ...
                agfs_path = ctx_fs._uri_to_path(target_uri)
                ctx_fs.agfs.write(agfs_path, encoded)
