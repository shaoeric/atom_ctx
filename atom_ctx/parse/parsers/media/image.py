...
"""
Media parser interfaces for AtomCtx - Future expansion.
...
from atom_ctx.parse.base import NodeType, ParseResult, ResourceNode
from atom_ctx.parse.parsers.base_parser import BaseParser
from atom_ctx.parse.parsers.media.constants import IMAGE_EXTENSIONS
...
class ImageParser(BaseParser):
    ...
    async def parse(self, source: Union[str, Path], instruction: str = "", **kwargs) -> ParseResult:
        ...
        ctx_fs = get_ctx_fs()
        temp_uri = ctx_fs.create_temp_uri()
        ...
        root_dir_name = CtxURI.sanitize_segment(f"{stem}_{ext_no_dot}")
        root_dir_uri = f"{temp_uri}/{root_dir_name}"
        await ctx_fs.mkdir(root_dir_uri, exist_ok=True)

        # 1.1 Save original image with original filename (sanitized)
        await ctx_fs.write_file_bytes(f"{root_dir_uri}/{original_filename}", image_bytes)
    ...
    async def _ocr_extract(self, image_bytes: bytes, lang: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning("pytesseract not installed. Install with: pip install atom-ctx[ocr]")
            return None
    ...
    async def _generate_semantic_info(
        self, node: ResourceNode, description: str, ctx_fs, has_ocr: bool, root_dir_uri: str
    ):
        """
        ...
        # await ctx_fs.write_file(f"{root_dir_uri}/.abstract.md", abstract)
        # await ctx_fs.write_file(f"{root_dir_uri}/.overview.md", overview)
