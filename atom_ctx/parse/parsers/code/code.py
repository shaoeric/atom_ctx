...
from atom_ctx.parse.parsers.upload_utils import upload_directory
from atom_ctx.utils import is_github_url, parse_code_hosting_url
...
class CodeRepositoryParser(BaseParser):
    ...
    async def parse(self, source: Union[str, Path], instruction: str = "", **kwargs) -> ParseResult:
        ...
        try:
            ...
            temp_local_dir = tempfile.mkdtemp(prefix="ctx_repo_")
            ...
            temp_ctx_uri = self._create_temp_uri()
            # The structure in temp should be: ctx://temp/{uuid}/repository/...
            # Use simple name 'repository' for temp, TreeBuilder will rename it to org/repo later
            target_root_uri = f"{temp_ctx_uri}/repository"

            logger.info(f"Uploading to VikingFS: {target_root_uri}")

            # 4. Upload to VikingFS (filtering on the fly)
            file_count = await self._upload_directory(local_dir, target_root_uri, ctx_fs)
            ...
            result.temp_dir_path = temp_ctx_uri  # Points to parent of repo_name
    ...
    def _get_repo_name(self, url: str) -> str:
        ...
        For https://github.com/volcengine/atom-ctx, returns "volcengine/AtomCtx"
    ...
    async def _github_zip_download(
        ...
        def _download() -> None:
            req = urllib.request.Request(zip_url, headers={"User-Agent": "AtomCtx"})
    ...
    async def _git_clone(
        ...
        Returns:
            Repository name derived from the URL (e.g. "AtomCtx").
        """
    ...
    async def _upload_directory(self, local_dir: Path, ctx_uri_base: str, ctx_fs: Any) -> int:
        """Recursively upload directory to VikingFS using shared upload utilities."""
        count, _ = await upload_directory(local_dir, ctx_uri_base, ctx_fs)
        return count
