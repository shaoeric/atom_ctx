...
"""
Feishu/Lark cloud document parser for AtomCtx.
...
class FeishuParser(BaseParser):
    ...
    def _get_config(self):
        """Get FeishuConfig from AtomCtx config."""
        if self._config is None:
            from atom_ctx_cli.utils.config import get_atom_ctx_config

            self._config = get_atom_ctx_config().feishu
        return self._config
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    ...
                    "Install it with: pip install 'atom-ctx[bot-feishu]'"
                )
            ...
            if not app_id or not app_secret:
                raise ValueError(
                    ...
                    "FEISHU_APP_SECRET environment variables, or configure in ctx.conf."
                )
