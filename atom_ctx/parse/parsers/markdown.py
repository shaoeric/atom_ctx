...
"""
Markdown parser for AtomCtx (v5.0).
...
class MarkdownParser(BaseParser):
    ...
    async def parse_content(
        ...
        **kwargs,
        ...
        Returns:
            ParseResult with temp_dir_path (Ctx URI)
        """
    ...
    async def _parse_and_create_structure(
        ...
        if estimated_tokens <= max_size and len(content) <= max_chars:
            ...
            await ctx_fs.write_file(file_path, content)
        ...
        if not headings:
            ...
            for part_idx, part in enumerate(parts, 1):
                await ctx_fs.write_file(f"{root_dir}/{doc_name}_{part_idx}.md", part)
        ...
        for sec in expanded:
            ...
            if tokens < min_size:
                pending = await self._try_add_to_pending(
                    ctx_fs, parent_dir, pending, (name, content_text, tokens), max_size
                )
                continue
            ...
            if pending and self._can_merge(pending, tokens, max_size, has_children):
                ...
                await self._save_merged(ctx_fs, parent_dir, pending)
                pending = []
                continue

            # Save pending and process current section
            pending = await self._flush_pending(ctx_fs, parent_dir, pending)
            await self._save_section(content, headings, parent_dir, sec, max_size, min_size)

        # Save remaining pending
        await self._flush_pending(ctx_fs, parent_dir, pending)
    ...
    async def _try_add_to_pending(
        self, ctx_fs, parent_dir: str, pending: List, item: Tuple, max_size: int
    ) -> List:
    ...
    async def _flush_pending(self, ctx_fs, parent_dir: str, pending: List) -> List:
    ...
    async def _save_section(
        ...
        if has_children:
            ...
            await self._split_content(ctx_fs, section_dir, name, content_text, max_size)
    ...
    async def _split_content(
        self, ctx_fs, section_dir: str, name: str, content: str, max_size: int
    ) -> None:
        """Split content by paragraphs."""
        logger.info(f"[MarkdownParser] Splitting: {name}")
        parts = self._smart_split_content(content, max_size)
        for i, part in enumerate(parts, 1):
            await ctx_fs.write_file(f"{section_dir}/{name}_{i}.md", part)
    ...
    async def _save_merged(
        self, ctx_fs, parent_dir: str, sections: List[Tuple[str, str, int]]
    ) -> None:
        ...
        if len(content) > max_chars:
            ...
            parts = self._smart_split_content(content, max_size)
            for i, part in enumerate(parts, 1):
                await ctx_fs.write_file(f"{parent_dir}/{name}_{i}.md", part)
            logger.debug(
                f"[MarkdownParser] Merged then split: {name} ({len(sections)} sections → {len(parts)} parts)"
            )
        else:
            await ctx_fs.write_file(f"{parent_dir}/{name}.md", content)
