...
from atom_ctx.parse.parsers.media.constants import MEDIA_EXTENSIONS
from atom_ctx_cli.utils.logger import get_logger

if TYPE_CHECKING:
    from atom_ctx.parse.directory_scan import ClassifiedFile
...
class DirectoryParser(BaseParser):
    ...
    async def parse(
        ...
        **kwargs,
        ...
        try:
            ...
            from atom_ctx.parse.directory_scan import scan_directory
            from atom_ctx.parse.registry import get_registry
            ...
            if preserve_structure is None:
                ...
                try:
                    ...
                    preserve_structure = get_atom_ctx_config().directory.preserve_structure
                except Exception:
            ...
            await ctx_fs.mkdir(temp_uri, exist_ok=True)
            await ctx_fs.mkdir(target_uri, exist_ok=True)
    ...
    async def _process_single_file(
        ...
        if parser:
            ...
            try:
                ...
                await ctx_fs.write_file(dst_uri, content)
                return True
            except Exception as exc:
    ...
    async def _merge_temp(
        ...
        """
        entries = await ctx_fs.ls(src_temp_uri)
        for entry in entries:
            ...
            if DirectoryParser._is_dir_entry(entry):
                await DirectoryParser._recursive_move(ctx_fs, src, dst)
            else:
                await ctx_fs.move_file(src, dst)
        try:
            await ctx_fs.delete_temp(src_temp_uri)
        except Exception:
            pass
    ...
    async def _recursive_move(
        ...
        await ctx_fs.mkdir(dst_uri, exist_ok=True)
        entries = await ctx_fs.ls(src_uri)
        for entry in entries:
            name = entry.get("name", "")
            if not name or name in (".", ".."):
                continue
            ...
            if DirectoryParser._is_dir_entry(entry):
                await DirectoryParser._recursive_move(ctx_fs, s, d)
            else:
                await ctx_fs.move_file(s, d)
