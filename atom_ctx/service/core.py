...
from atom_ctx.core.directories import DirectoryInitializer
...
from atom_ctx.service.debug_service import DebugService
...
from atom_ctx.session import create_session_compressor, SessionCompressor
from atom_ctx.storage import VikingDBManager
from atom_ctx.storage.collection_schemas import init_context_collection
from atom_ctx.storage.queuefs.queue_manager import QueueManager, init_queue_manager
from atom_ctx.storage.transaction import LockManager, init_lock_manager
from atom_ctx.storage.ctx_fs import VikingFS, init_ctx_fs
from atom_ctx.utils.resource_processor import ResourceProcessor
...
from atom_ctx_cli.utils.config.ctx_config import initialize_atom_ctx_config
from atom_ctx_cli.utils.config.storage_config import StorageConfig

logger = get_logger(__name__)


class AtomCtxService:
    """
    AtomCtx main service class.
    ...
    def __init__(
        ...
        """Initialize AtomCtx service.

        Args:
            path: Local storage path (overrides ctx.conf storage path).
            user: Username for session management.
        """
        # Initialize config from ctx.conf
        config = initialize_atom_ctx_config(
            user=user,
            path=path,
        )
        self._config = config
        ...
        self._ctx_fs: Optional[VikingFS] = None
    ...
    @property
    def ctx_fs(self) -> Optional[VikingFS]:
    ...
    async def initialize(self) -> None:
        """Initialize AtomCtx storage and indexes."""
        if self._initialized:
            logger.debug("Already initialized")
            return

        # Acquire advisory lock on data directory to prevent multi-process
        # contention (see https://github.com/volcengine/atom-ctx/issues/473).
        from atom_ctx.utils.process_lock import acquire_data_dir_lock
        ...
        enable_recorder = os.environ.get("ATOM_CTX_ENABLE_RECORDER", "").lower() == "true"
        ...
        self._fs_service.set_ctx_fs(self._ctx_fs)
        self._relation_service.set_ctx_fs(self._ctx_fs)
        self._pack_service.set_ctx_fs(self._ctx_fs)
        self._search_service.set_ctx_fs(self._ctx_fs)
        ...
        logger.info("AtomCtxService initialized")

    async def close(self) -> None:
        """Close AtomCtx and release resources."""
        ...
        logger.info("AtomCtxService closed")

    def _ensure_initialized(self) -> None:
        """Ensure service is initialized."""
        if not self._initialized:
            raise NotInitializedError("AtomCtxService")
