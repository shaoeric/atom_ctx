...
from atom_ctx_cli.exceptions import NotFoundError, NotInitializedError
...
class SessionService:
    ...
    async def sessions(self, ctx: RequestContext) -> List[Dict[str, Any]]:
        ...
        """
        self._ensure_initialized()
        session_base_uri = f"ctx://session/{ctx.user.user_space_name()}"

        try:
            entries = await self._ctx_fs.ls(session_base_uri, ctx=ctx)
            sessions = []
    ...
    async def delete(self, session_id: str, ctx: RequestContext) -> bool:
        ...
        """
        self._ensure_initialized()
        session_uri = f"ctx://session/{ctx.user.user_space_name()}/{session_id}"

        try:
            await self._ctx_fs.rm(session_uri, recursive=True, ctx=ctx)
