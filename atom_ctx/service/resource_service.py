...
from atom_ctx.telemetry.resource_summary import (
...
class ResourceService:
    ...
    async def add_resource(
        ...
        **kwargs,
        ...
        """Add resource to AtomCtx (only supports resources scope).

        Args:
            path: Resource path (local file or URL)
            to: Target URI (e.g., "ctx://resources/my_resource")
        ...
        try:
            ...
            if parent and parent.startswith("ctx://"):
                parsed = CtxURI(parent)
                if parsed.scope != "resources":
                    raise InvalidArgumentError(
                        f"add_resource only supports resources scope, use dedicated interface to add {parsed.scope} content"
                    )
