...
"""
Storage path management for AtomCtx.

Manages file storage in .ctx/ directory for media files (images, tables, etc.).
"""
...
class StoragePath:
    """
    Manages .ctx/ directory for storing extracted media files.
    ...
    def __init__(self, base_path: Optional[Path] = None):
        ...
        Args:
            base_path: Base directory for .ctx folder.
                      If None, uses current working directory.
        """
        self.base_path = Path(base_path) if base_path else Path.cwd()
        self.ctx_dir = self.base_path / self.BASE_DIR
        self.media_dir = self.ctx_dir / self.MEDIA_DIR
        self.downloads_dir = self.ctx_dir / self.DOWNLOADS_DIR
    ...
    def cleanup_all(self) -> None:
        """Remove all AtomCtx storage."""
        if self.ctx_dir.exists():
            shutil.rmtree(self.ctx_dir)
            logger.info("Cleaned up all AtomCtx storage")
