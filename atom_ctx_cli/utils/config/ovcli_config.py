...
"""Configuration schema and loader for ctx-cli.conf."""
...
from .consts import DEFAULT_CTX_CLI_CONF, CTX_CLI_CONFIG_ENV


class OVCLIConfig(BaseModel):
    """Client configuration loaded from ctx-cli.conf."""
...
def load_ovcli_config(config_path: Optional[str] = None) -> Optional[OVCLIConfig]:
    """Load ctx-cli.conf if present and validate it strictly."""
    path = resolve_config_path(config_path, CTX_CLI_CONFIG_ENV, DEFAULT_CTX_CLI_CONF)
    if path is None:
        return None
