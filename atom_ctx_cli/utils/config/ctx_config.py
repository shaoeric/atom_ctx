...
class AtomCtxConfig(BaseModel):
    """Main configuration for AtomCtx."""
    ...
    @classmethod
    def from_dict(cls, config: Dict[str, Any]) -> "AtomCtxConfig":
...
class AtomCtxConfigSingleton:
    """Global singleton for AtomCtxConfig.

    Resolution chain for ctx.conf:
      1. Explicit path passed to initialize()
      2. CTX_CONFIG_FILE environment variable
      3. ~/.ctx/ctx.conf
      4. /etc/ctx/ctx.conf
      5. Error with clear guidance
    """

    _instance: Optional[AtomCtxConfig] = None
    _lock: Lock = Lock()

    @classmethod
    def get_instance(cls) -> AtomCtxConfig:
        ...
        if cls._instance is None:
            with cls._lock:
                if cls._instance is None:
                    config_path = resolve_config_path(None, CTX_CONFIG_ENV, DEFAULT_CTX_CONF)
                    if config_path is not None:
                        cls._instance = cls._load_from_file(str(config_path))
                    else:
                        default_path_user = DEFAULT_CONFIG_DIR / DEFAULT_CTX_CONF
                        default_path_system = SYSTEM_CONFIG_DIR / DEFAULT_CTX_CONF
                        raise FileNotFoundError(
                            f"AtomCtx configuration file not found.\n"
                            f"Please create {default_path_user} or {default_path_system}, or set {CTX_CONFIG_ENV}.\n"
                            f"See: https://atom_ctx.dev/docs/guides/configuration"
                        )
    ...
    def initialize(
        ...
        Args:
            ...
            config_path: Explicit path to ctx.conf file.
        """
        with cls._lock:
            if config_dict is not None:
                cls._instance = AtomCtxConfig.from_dict(config_dict)
            else:
                path = resolve_config_path(config_path, CTX_CONFIG_ENV, DEFAULT_CTX_CONF)
                if path is not None:
    ...
    @classmethod
    def _load_from_file(cls, config_file: str) -> "AtomCtxConfig":
        ...
        try:
            ...
            return AtomCtxConfig.from_dict(config_data)
...
def get_atom_ctx_config() -> AtomCtxConfig:
    """Get the global AtomCtxConfig instance."""
    return AtomCtxConfigSingleton.get_instance()


def set_atom_ctx_config(config: AtomCtxConfig) -> None:
    """Set the global AtomCtxConfig instance."""
    AtomCtxConfigSingleton.initialize(config_dict=config.to_dict())


def is_valid_atom_ctx_config(config: AtomCtxConfig) -> bool:
    """
    Check if AtomCtxConfig is valid.
    ...
    if errors:
        error_message = "Invalid AtomCtx configuration:\n" + "\n".join(
            f"  - {e}" for e in errors
        )
...
def initialize_atom_ctx_config(
    user: Optional[UserIdentifier] = None,
    path: Optional[str] = None,
) -> AtomCtxConfig:
    """
    Initialize AtomCtx configuration with provided parameters.

    Loads ctx.conf from the standard resolution chain, then applies
    parameter overrides.
    ...
    Returns:
        Configured AtomCtxConfig instance
    ...
    if not is_valid_atom_ctx_config(config):
        raise ValueError("Invalid AtomCtx configuration")
