...
def resolve_config_path(
    ...
    Resolution order:
      ...
      3. ``~/.ctx/<default_filename>``
      4. ``/etc/ctx/<default_filename>``
    ...
    # Level 3: default directory (~/.ctx)
    p = DEFAULT_CONFIG_DIR / default_filename
    if p.exists():
        return p

    # Level 4: system directory (/etc/ctx)
...
def require_config(
    ...
    Args:
        ...
        default_filename: Default filename under ~/.ctx/.
    ...
    if path is None:
        ...
        raise FileNotFoundError(
            f"AtomCtx {purpose} configuration file not found.\n"
