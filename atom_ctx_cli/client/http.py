...
"""Async HTTP Client for AtomCtx.

Implements BaseClient interface using HTTP calls to AtomCtx Server.
"""
...
from atom_ctx.telemetry import TelemetryRequest, normalize_telemetry_request
...
from atom_ctx_cli.utils import run_async
from atom_ctx_cli.utils.config.ovcli_config import load_ovcli_config
...
class AsyncHTTPClient(BaseClient):
    """Async HTTP Client for AtomCtx Server.

    Implements BaseClient interface using HTTP calls.
    Supports auto-loading url/api_key from ctx-cli.conf when not provided.

    Examples:
        # Explicit url
        client = AsyncHTTPClient(url="http://localhost:1933", api_key="key")

        # Auto-load from ~/.ctx/ctx-cli.conf
    ...
    def __init__(
        ...
        Args:
            url: AtomCtx Server URL. If not provided, reads from ctx-cli.conf.
            api_key: API key for authentication. If not provided, reads from ctx-cli.conf.
            user_id: User identifier. If not provided, defaults to "default".
            agent_id: Agent identifier. If not provided, reads from ctx-cli.conf.
            account: Account identifier for multi-tenant auth. Required when using root key
                     to access tenant-scoped APIs. If not provided, reads from ctx-cli.conf.
    ...
    async def initialize(self) -> None:
        ...
        if self._account:
            headers["X-AtomCtx-Account"] = self._account
        if self._user_id:
            headers["X-AtomCtx-User"] = self._user_id
    ...
    def _raise_exception(self, error: Dict[str, Any]) -> None:
        ...
        exc_class = ERROR_CODE_TO_EXCEPTION.get(code, AtomCtxError)
    ...
    async def mv(self, from_uri: str, to_uri: str) -> None:
        """Move resource."""
        from_uri = CtxURI.normalize(from_uri)
        to_uri = CtxURI.normalize(to_uri)
        response = await self._http.post(
            "/api/v1/fs/mv",
    ...
    async def glob(self, pattern: str, uri: str = "ctx://") -> Dict[str, Any]:
    ...
    async def link(self, from_uri: str, to_uris: Union[str, List[str]], reason: str = "") -> None:
        ...
        if isinstance(to_uris, str):
            to_uris = CtxURI.normalize(to_uris)
        else:
            to_uris = [CtxURI.normalize(u) for u in to_uris]
        response = await self._http.post(
    ...
    async def export_ctxpack(self, uri: str, to: str) -> str:
    ...
    async def import_ctxpack(
        ...
        """Import .ctxpack file."""
        parent = CtxURI.normalize(parent)
        request_data = {
            "parent": parent,
            "force": force,
            "vectorize": vectorize,
