...
def main():
    ...
    1. ./target/release/ctx（开发环境）
    2. Wheel 自带：{package_dir}/atom_ctx/bin/ctx
    3. PATH 查找：系统全局安装的 ctx
    """
    # 0. Python-native subcommands (no Rust binary needed)
    if len(sys.argv) > 1 and sys.argv[1] == "doctor":
        from atom_ctx_cli.doctor import main as doctor_main

        sys.exit(doctor_main())
    # 1. 检查开发环境（仅在直接运行脚本时有效）
    try:
        dev_binary = Path(__file__).parent.parent / "target" / "release" / "ctx"
        if dev_binary.exists() and os.access(dev_binary, os.X_OK):
            _exec_binary(str(dev_binary), sys.argv[1:])
    except Exception:
        pass

    # 2. 检查 Wheel 自带（不导入 atom_ctx，避免额外开销）
    try:
        package_dir = Path(__file__).parent.parent / "atom_ctx"
        package_bin = package_dir / "bin"
        for binary_name in ["ctx", "ctx.exe"]:
   ...
   curl -fsSL https://raw.githubusercontent.com/volcengine/atom-ctx/main/crates/ctx_cli/install.sh | bash

        3. 从 GitHub Releases 下载（零 Python 开销）：
   https://github.com/volcengine/atom-ctx/releases
