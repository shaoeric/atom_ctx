...
"""ctx doctor - validate AtomCtx subsystems and report actionable diagnostics.

Unlike ``ctx health`` (which pings a running server), ``ctx doctor`` checks
...
from atom_ctx_cli.utils.config.config_loader import resolve_config_path
from atom_ctx_cli.utils.config.consts import CTX_CONFIG_ENV
...
def _find_config() -> Optional[Path]:
    return resolve_config_path(None, CTX_CONFIG_ENV, "ctx.conf")


def _load_config_json(config_path: Path) -> Optional[dict]:
    """Parse ctx.conf as JSON. Returns None if the file is unreadable or not valid JSON."""
    try:
...
def check_config() -> tuple[bool, str, Optional[str]]:
    """Validate ctx.conf exists and is valid JSON with required sections."""
    config_path = _find_config()
    if config_path is None:
        return (
            False,
            "Configuration file not found",
            f"Create ~/.ctx/ctx.conf or set {CTX_CONFIG_ENV}",
        )
    ...
    if missing:
        return (
            ...
            "Add the missing sections (see examples/ctx.conf.example)",
        )
...
def check_native_engine() -> tuple[bool, str, Optional[str]]:
    ...
    try:
        from atom_ctx.storage.vectordb.engine import (
...
def check_agfs() -> tuple[bool, str, Optional[str]]:
    """Verify the bundled AtomCtx AGFS client loads."""
    try:
        pyagfs = importlib.import_module("atom_ctx.pyagfs")
...
def check_embedding() -> tuple[bool, str, Optional[str]]:
    ...
    if provider == "unknown":
        return False, "No embedding provider configured", "Add embedding.dense section to ctx.conf"
    ...
    if not api_key or api_key.startswith("{"):
        return (
            ...
            "Set embedding.dense.api_key in ctx.conf",
        )
...
def check_vlm() -> tuple[bool, str, Optional[str]]:
    ...
    if not provider:
        return False, "No VLM provider configured", "Add vlm section to ctx.conf"
...
def check_disk() -> tuple[bool, str, Optional[str]]:
    ...
    if free_gb < 1.0:
        return (
            ...
            "Free up disk space (AtomCtx needs at least 1 GB for vector storage)",
        )
