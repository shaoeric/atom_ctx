...
|--------|--------------------------------------|-------------------------------------------|
| **Architecture** | Bash hooks + Python bridge (`ctx_memory.py`) | Native MCP server + Node.js hooks |
...
| **Config Scope** | Project-local (`./ctx.conf`) | Global (`~/.ctx/ctx.conf`) — works across all projects |
...
4. **Global Configuration**: One config file for all projects (`~/.ctx/ctx.conf`), not per-project `./ctx.conf` files.
...
1. **VLM Token Cost**: Each capture triggers an LLM extraction call to AtomCtx. Capturing every assistant turn (including tool calls, sub-agent outputs, etc.) quickly becomes expensive.
...
Users can opt-in to full capture by setting `"captureAssistantTurns": true` in their `ctx.conf` if they want assistant turns included (e.g., for non-coding workflows where assistant insights matter).
...
/plugin uninstall atom_ctx-memory
...
Add the `claude_code` section to `~/.ctx/ctx.conf` for plugin-specific settings:
...
| No memories recalled | Ensure AtomCtx server is running: `ctx-server` |
