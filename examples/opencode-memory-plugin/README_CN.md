# OpenCode 的 AtomCtx 记忆插件

OpenCode 插件示例，将 AtomCtx 记忆作为显式工具暴露，并自动同步对话会话到 AtomCtx。

安装指南：[INSTALL-ZH.md](./INSTALL-ZH.md)

## 机制

本示例使用 OpenCode 的工具机制将 AtomCtx 功能暴露为agent可调用的显式工具。

实际上，这意味着：

- agent可以看到具体的工具并决定何时调用它们
- AtomCtx 数据通过工具执行按需获取，而不是预先注入到每个提示中
- 插件还保持 AtomCtx 会话与 OpenCode 对话同步，并使用 `memcommit` 触发后台记忆提取
...
- 为 OpenCode agent暴露四个记忆工具：
  ...
  - `membrowse`
  - `memcommit`
- 自动将每个 OpenCode 会话映射到 AtomCtx 会话
- 将用户和助手消息流式传输到 AtomCtx
...
- `atom_ctx-memory.ts`：OpenCode 使用的插件实现
- `atom_ctx-config.example.json`：配置模板
...
- OpenCode
- AtomCtx HTTP 服务器
- 如果您的服务器需要身份验证，则需要有效的 AtomCtx API 密钥
...
然后编辑 `~/.config/opencode/plugins/atom_ctx-config.json`。
...
- `query`：搜索查询
- `target_uri?`：将搜索限制在 URI 前缀，如 `ctx://user/memories/`
- `mode?`：`auto | fast | deep`
- `limit?`：最大结果数
...
- `session_id?`：可选的显式 AtomCtx 会话 ID
...
- 使用 AtomCtx 后台提交任务来避免长时间记忆提取期间的重复超时/重试循环

## 故障排除

- 插件未加载：确认文件存在于 `~/.config/opencode/plugins/atom_ctx-memory.ts`
- 服务不可用：确认 `ctx-server` 正在运行并且在配置的端点可访问
- 身份验证失败：检查 `ATOM_CTX_API_KEY` 或 `atom_ctx-config.json`
- 未提取记忆：检查您的 AtomCtx 服务器是否有正常工作的 `vlm` 和 `embedding` 配置
