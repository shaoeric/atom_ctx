 ...
 *
 * Exposes AtomCtx's semantic memory capabilities as tools for AI agents.
...
// Map: OpenCode session ID → AtomCtx session ID
...
function initLogger() {
  ...
  logFilePath = path.join(pluginDir, "atom_ctx-memory.log")
}
...
function initSessionMapPath() {
  ...
  if (!pluginDir) return
  pluginDataDir = pluginDir
  sessionMapPath = path.join(pluginDir, "atom_ctx-session-map.json")
}
...
interface AtomCtxResponse<T = unknown> {
  status: string
  result?: T
...
const DEFAULT_CONFIG: AtomCtxConfig = {
...
function loadConfig(): AtomCtxConfig {
  const configPath = path.join(pluginFileDir, "atom_ctx-config.json")
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      if (process.env.ATOM_CTX_API_KEY) {
        config.apiKey = process.env.ATOM_CTX_API_KEY
      }

      return config
    }
  } catch (error) {
    console.warn(`Failed to load AtomCtx config from ${configPath}:`, error)
  }
...
async function makeRequest<T = any>(config: AtomCtxConfig, options: HttpRequestOptions): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        case 500:
          throw new Error(`AtomCtx server error: ${errorMessage}`)
        default:
    ...
    if (error.message?.includes("fetch failed") || error.code === "ECONNREFUSED") {
      throw new Error(
        `AtomCtx service unavailable at ${config.endpoint}. Please check if the service is running (try: ctx-server).`,
      )
    }
...
function getResponseErrorMessage(error: AtomCtxResponse["error"]): string {
  if (!error) return "Unknown AtomCtx error"
  if (typeof error === "string") return error
  return error.message || error.code || "Unknown AtomCtx error"
}

function unwrapResponse<T>(response: AtomCtxResponse<T>): T {
  if (!response || typeof response !== "object") {
    throw new Error("AtomCtx returned an invalid response")
  }
...
async function checkServiceHealth(config: AtomCtxConfig): Promise<boolean> {
  try {
    ...
    log("ERROR", "health", "AtomCtx health check failed", {
...
function getAutoCommitIntervalMinutes(config: AtomCtxConfig): number {
...
/**
 * Create or connect to AtomCtx session for an OpenCode session
 */
async function ensureAtomCtxSession(
  opencodeSessionId: string,
  config: AtomCtxConfig,
): Promise<string | null> {
  ...
  if (knownSessionId) {
    try {
      const response = await makeRequest<AtomCtxResponse<SessionResult>>(config, {
        method: "GET",
      ...
      if (result) {
        log("INFO", "session", "Reconnected to persisted AtomCtx session", {
          opencode_session: opencodeSessionId,
          atom_ctx_session: knownSessionId,
        })
        return knownSessionId
      }
    } catch (error: any) {
      log("INFO", "session", "Persisted AtomCtx session unavailable, creating a new one", {
        opencode_session: opencodeSessionId,
        atom_ctx_session: knownSessionId,
        error: error.message,
      })
  ...
  try {
    const createResponse = await makeRequest<AtomCtxResponse<SessionResult>>(config, {
      method: "POST",
    ...
    if (!sessionId) {
      throw new Error("AtomCtx did not return a session_id")
    }

    log("INFO", "session", "Created new AtomCtx session", {
      opencode_session: opencodeSessionId,
      atom_ctx_session: sessionId,
    })
    return sessionId
  } catch (error: any) {
    log("ERROR", "session", "Failed to create AtomCtx session", {
      opencode_session: opencodeSessionId,
      error: error.message,
    })
    return null
...
async function findRunningCommitTaskId(
  ...
  try {
    const response = await makeRequest<AtomCtxResponse<TaskResult[]>>(config, {
      method: "GET",
...
async function detectBackgroundCommitSupport(config: AtomCtxConfig): Promise<boolean> {
...
async function finalizeCommitSuccess(
  ...
  if (mapping.pendingCleanup) {
    ...
    log("INFO", "session", "Cleaned up session mapping after commit completion", {
      atom_ctx_session: mapping.ovSessionId,
      opencode_session: opencodeSessionId,
    })
  }
}
...
async function runSynchronousCommit(
  ...
  try {
    const response = await makeRequest<AtomCtxResponse<CommitResult>>(config, {
      method: "POST",
    ...
    log("INFO", "session", "AtomCtx synchronous commit completed", {
      atom_ctx_session: mapping.ovSessionId,
      opencode_session: opencodeSessionId,
...
async function startBackgroundCommit(
  ...
  try {
    ...
    if (!taskId) {
      throw new Error("AtomCtx did not return a background task id")
    }
    ...
    log("INFO", "session", "AtomCtx background commit accepted", {
      atom_ctx_session: mapping.ovSessionId,
      opencode_session: opencodeSessionId,
      task_id: taskId,
    })
    ...
    log("ERROR", "session", "Failed to start AtomCtx background commit", {
      atom_ctx_session: mapping.ovSessionId,
      opencode_session: opencodeSessionId,
      error: error.message,
    })
...
async function pollCommitTaskOnce(
  ...
  try {
    ...
    if (task.status === "completed") {
      ...
      log("INFO", "session", "AtomCtx background commit completed", {
        atom_ctx_session: mapping.ovSessionId,
        opencode_session: opencodeSessionId,
    ...
    log("ERROR", "session", "AtomCtx background commit failed", {
      atom_ctx_session: mapping.ovSessionId,
      opencode_session: opencodeSessionId,
      task_id: task.task_id,
      error: task.error,
    ...
    log("ERROR", "session", "Failed to poll AtomCtx background commit", {
      atom_ctx_session: mapping.ovSessionId,
      opencode_session: opencodeSessionId,
...
function startAutoCommit(config: AtomCtxConfig) {
...
async function checkAndCommitSessions(config: AtomCtxConfig): Promise<void> {
...
/**
 * Add message to AtomCtx session
 */
async function addMessageToSession(
  ...
  try {
    const response = await makeRequest<AtomCtxResponse<void>>(config, {
      method: "POST",
    ...
    log("INFO", "message", "Message added to AtomCtx session", {
      atom_ctx_session: ovSessionId,
      role,
      content_length: content.length,
    })
    ...
    log("ERROR", "message", "Failed to add message to AtomCtx session", {
      atom_ctx_session: ovSessionId,
      role,
      error: error.message,
    })
...
function validateVikingUri(uri: string, toolName: string): string | null {
  if (!uri.startsWith("ctx://")) {
    const error = `Invalid URI format. Must start with "ctx://". Example: ctx://user/memories/`
...
export const AtomCtxMemoryPlugin = async (input: PluginInput): Promise<Hooks> => {
  const config = loadConfig()
  initLogger()
  initSessionMapPath()

  if (!config.enabled) {
    console.log("AtomCtx Memory Plugin is disabled in configuration")
    return {}
  }

  log("INFO", "plugin", "AtomCtx Memory Plugin initialized", { endpoint: config.endpoint })

  // Load session map from disk
  await loadSessionMap()

  const healthy = await checkServiceHealth(config)
  log("INFO", "health", healthy ? "AtomCtx health check passed" : "AtomCtx health check failed", {
    endpoint: config.endpoint,
  })
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.created") {
        ...
        // Create or connect to AtomCtx session (non-blocking)
        const ovSessionId = await ensureAtomCtxSession(sessionId, config)
        if (ovSessionId) {
        ...
        // Commit AtomCtx session if mapped
        const mapping = sessionMap.get(sessionId)
        if (mapping) {
          await flushPendingMessages(sessionId, mapping, config)
    ...
    tool: {
      memread: tool({
        description:
          "Retrieve the content of a specific memory, resource, or skill at a given ctx:// URI.\n\nProgressive loading levels:\n- abstract: brief summary\n- overview: structured directory overview\n- read: full content\n- auto: choose overview for directories and read for files\n\nUse when:\n- You have a URI from memsearch or membrowse\n- You need to inspect a memory, resource, or skill in more detail\n\nRequires: Complete ctx:// URI (e.g., ctx://user/memories/profile.md)",
        args: {
          uri: z
            .string()
            .describe(
              "Complete ctx:// URI from search results or list output (e.g., ctx://user/memories/profile.md, ctx://agent/memories/context.md)",
            ),
          level: z
        ...
        async execute(args, context) {
          ...
          try {
            ...
            if (level === "auto") {
              try {
                const statResponse = await makeRequest<AtomCtxResponse<{ isDir?: boolean }>>(config, {
                  method: "GET",
            ...
            const response = await makeRequest<AtomCtxResponse<string | Record<string, unknown>>>(config, {
              method: "GET",
      ...
      membrowse: tool({
        ...
        async execute(args, context) {
          ...
          try {
            ...
            if (view === "stat") {
              const response = await makeRequest<AtomCtxResponse<Record<string, unknown>>>(config, {
                method: "GET",
            ...
            const response = await makeRequest<AtomCtxResponse<any[]>>(config, {
              method: "GET",
              endpoint,
              abortSignal: context.abort,
            })
      ...
      memcommit: tool({
        description:
          "Commit the current OpenCode session to AtomCtx and extract persistent memories from the accumulated conversation.\n\nBy default this tool commits the AtomCtx session mapped to the current OpenCode session. Use `session_id` only when you need to target a specific AtomCtx session manually.\n\nUse when:\n- You want a mid-session memory extraction without ending the chat\n- You want recently discussed preferences, entities, or cases persisted immediately\n\nAutomatically extracts and stores:\n- User profile, preferences, entities, events → ctx://user/memories/\n- Agent cases and patterns → ctx://agent/memories/\n\nReturns background commit progress or completion details, including task_id, memories_extracted, and archived.",
        args: {
          session_id: z
            .string()
            .optional()
            .describe("Optional explicit AtomCtx session ID. Omit to commit the current OpenCode session's mapped AtomCtx session."),
        },
        async execute(args, context) {
          ...
          if (!sessionId) {
            return "Error: No AtomCtx session is associated with the current OpenCode session. Start or resume a normal OpenCode session first, or pass an explicit session_id."
          }
      ...
      memsearch: tool(
        {
          ...
          args: {
            ...
            target_uri: z
              ...
              .optional()
              .describe(
                "Limit search to a specific URI prefix (e.g., ctx://resources/, ctx://user/memories/). Omit to search all contexts.",
              ),
            mode: z
              .enum(["auto", "fast", "deep"])
              .optional()
            ...
            session_id: z
              ...
              .optional()
              .describe(
                "Optional AtomCtx session ID for context-aware search. If omitted in auto/deep mode, the current OpenCode session mapping will be used when available.",
              ),
          ...
          async execute(args, context) {
            ...
            try {
              const response = await makeRequest<AtomCtxResponse<SearchResult>>(config, {
                method: "POST",
    ...
    stop: async () => {
      ...
      stopAutoCommit()
      log("INFO", "plugin", "AtomCtx Memory Plugin stopped")
    }
  }
}

export default AtomCtxMemoryPlugin
