...
MCP（Model Context Protocol）HTTP 服务器，将 AtomCtx RAG 能力作为工具暴露。
...
uv run server.py [OPTIONS]
  ...
  --config PATH       配置文件路径（默认：./ctx.conf，环境变量：CTX_CONFIG）
  --data PATH         数据目录路径（默认：./data，环境变量：CTX_DATA）
  --host HOST         绑定地址（默认：127.0.0.1）
  --port PORT         监听端口（默认：2033，环境变量：CTX_PORT）
