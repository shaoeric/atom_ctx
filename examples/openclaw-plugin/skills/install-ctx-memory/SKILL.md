...
description: AtomCtx long-term memory plugin guide. Once installed, the plugin automatically remembers important facts from conversations and recalls relevant context before responding.
---

# AtomCtx Memory Guide

## How It Works

- **Auto-Capture**: At `afterTurn` (end of one user turn run), automatically extracts memories from user/assistant messages
  - `semantic` mode: captures all qualifying user text, relying on AtomCtx's extraction pipeline to filter
...
Searches long-term memories in AtomCtx, returns relevant results.
...
Writes text to an AtomCtx session and runs memory extraction.

| Parameter | Required | Description |
|-----------|----------|-------------|
| `text` | Yes | Information text to store |
| `role` | No | Session role (default `user`) |
| `sessionId` | No | Existing AtomCtx session ID |
...
| `agentId` | `default` | Identifies this agent to AtomCtx |
| `configPath` | `~/.ctx/ctx.conf` | Config file path (local mode) |
| `port` | `1933` | Local server port (local mode) |
| `targetUri` | `ctx://user/memories` | Default search scope |
