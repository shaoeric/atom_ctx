# OpenClaw + AtomCtx 上下文引擎插件

使用 [AtomCtx](https://github.com/volcengine/atom-ctx) 作为 [OpenClaw](https://github.com/openclaw/openclaw) 的长期记忆后端。在 OpenClaw 中，此插件注册为 `atom_ctx` 上下文引擎。安装后，OpenClaw 将自动**记忆**对话中的重要信息，并在响应前**召回**相关上下文。

> **ℹ️ 历史兼容性说明**
>
> 传统的 AtomCtx/OpenClaw 集成在 OpenClaw `2026.3.12` 附近存在一个已知问题，即对话可能在插件加载后挂起。
> 该问题仅影响传统插件路径；本文档中描述的当前上下文引擎插件 2.0 不受此影响，因此新安装无需因此降级 OpenClaw。
> 插件 2.0 也不向后兼容传统的 `memory-atom_ctx` 插件及其配置，因此升级必须替换旧设置，而不是混合使用两个版本。
> 插件 2.0 还依赖于 OpenClaw 的上下文引擎功能，不支持旧版 OpenClaw；使用此插件前请先升级 OpenClaw。
> 如果您正在排查传统部署的问题，请参阅 [#591](https://github.com/volcengine/atom-ctx/issues/591) 和上游修复 PR：openclaw/openclaw#34673、openclaw/openclaw#33547。

> **🚀 插件 2.0（上下文引擎架构）**
>
> 本文档涵盖当前基于上下文引擎架构的 AtomCtx 插件 2.0，这是 AI 编码助手的推荐集成路径。
> 有关设计背景和早期讨论，请参阅：
...
3. **选择部署模式** — 本地或远程（见下文）
4. **生成配置** — 自动写入 `~/.ctx/ctx.conf` 和 `~/.openclaw/atom_ctx.env`
...
环境变量：
  ATOM_CTX_PYTHON       Python 路径
  CTX_CONFIG_FILE  ctx.conf 路径
  ATOM_CTX_REPO         本地 AtomCtx 仓库路径
  ATOM_CTX_ARK_API_KEY  火山引擎 API Key（-y 模式下跳过提示）
```
...
生成的配置文件：
- `~/.ctx/ctx.conf` — AtomCtx 服务配置
- `~/.openclaw/atom_ctx.env` — 环境变量（Python 路径等）
...
> 在本地模式下，您必须先 `source` 环境文件 — 插件会自动启动一个 AtomCtx子进程。

#### 步骤 4：验证

```bash
openclaw status
# ContextEngine 行应显示：enabled (plugin atom_ctx)
```
...
适用于多个OpenClaw实例或团队使用。部署一个独立的AtomCtx服务，供多个agents共享。**客户端无需 Python/AtomCtx。**

#### 步骤 1：部署 AtomCtx 服务

编辑 `~/.ctx/ctx.conf` — 设置 `root_api_key` 以启用多租户：
...
在火山引擎 ECS 上部署 OpenClaw + AtomCtx。详见 [火山引擎文档](https://www.volcengine.com/docs/6396/2249500?lang=zh)。
...
| `baseUrl` | `http://127.0.0.1:1933` | AtomCtx 服务器 URL（远程模式） |
| `apiKey` | — | AtomCtx API Key（可选） |
| `agentId` | 自动生成 | agent标识符，用于区分 OpenClaw 实例。如果未设置则自动生成 `openclaw-<hostname>-<random>` |
| `configPath` | `~/.ctx/ctx.conf` | 配置文件路径（本地模式） |
| `port` | `1933` | 本地服务器端口（本地模式） |
| `targetUri` | `ctx://user/memories` | 默认记忆搜索范围 |
...
AtomCtx 提供 Web 控制台用于调试和检查存储的记忆。
...
| 症状 | 原因 | 解决方案 |
|---------|-------|-----|
| 对话挂起，无响应 | 通常是受历史 OpenClaw `2026.3.12` 问题影响的 2.0 之前传统集成 | 如果您使用传统路径，请参阅 [#591](https://github.com/volcengine/atom-ctx/issues/591) 并临时降级到 `2026.3.11`；对于当前安装，请迁移到插件 2.0 |
| 日志中出现 `registerContextEngine is unavailable` | OpenClaw 版本过旧，未暴露插件 2.0 所需的上下文引擎 API | 升级 OpenClaw 到当前版本，然后重启网关并验证 `openclaw status` 显示 `atom_ctx` 作为 ContextEngine |
| agent静默挂起，无输出 | 自动召回缺少超时保护 | 临时禁用自动召回：`openclaw config set plugins.entries.ctx.config.autoRecall false --json`，或应用 [#673](https://github.com/volcengine/atom-ctx/issues/673) 中的补丁 |
| ContextEngine 不是 `atom_ctx` | 插件插槽未配置 | `openclaw config set plugins.slots.contextEngine atom_ctx` |
| `memory_store failed: fetch failed` | AtomCtx 未运行 | 检查 `ctx.conf` 和 Python 路径；验证服务是否运行 |
| `health check timeout` | 端口被陈旧进程占用 | `lsof -ti tcp:1933 \| xargs kill -9`，然后重启 |
| `extracted 0 memories` | API Key 或模型名称错误 | 检查 `ctx.conf` 中的 `api_key` 和 `model` |
| `port occupied` | 端口被其他进程占用 | 更改端口：`openclaw config set plugins.entries.ctx.config.port 1934` |
| 插件未加载 | 环境文件未 source | 启动前运行 `source ~/.openclaw/atom_ctx.env` |
