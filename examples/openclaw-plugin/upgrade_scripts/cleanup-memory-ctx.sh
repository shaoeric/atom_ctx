#!/bin/bash
#
# 从旧版 memory-atom_ctx 插件升级到新版 atom_ctx 的前置清理脚本
#
# 用法：
#   bash cleanup-legacy-atom_ctx.sh
#   bash cleanup-legacy-atom_ctx.sh --workdir ~/.openclaw-second  # 指定 OpenClaw 目录
#
set -euo pipefail
...
LEGACY_PLUGIN_ID="memory-atom_ctx"
BAK_SUFFIX=".pre-atom_ctx-upgrade.bak"
...
#
# 注意：openclaw config 命令无法处理带连字符的 key（如 memory-atom_ctx），
...
info "✓ 前置清理完成"
info ""
info "下一步：安装新版 atom_ctx 插件"
info "  npm install -g openclaw-atom_ctx-setup-helper && ov-install"
info ""
info "如需回滚，恢复备份："
