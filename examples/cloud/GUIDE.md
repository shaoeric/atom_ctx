...
本文档介绍如何将 AtomCtx 部署到火山引擎云上，使用 TOS（对象存储）+ VikingDB（向量数据库）+ 方舟大模型作为后端。
...
- 已安装 AtomCtx（`pip install atom-ctx --upgrade --force-reinstall` 或从源码安装）
- Python 3.11+
...
TOS 用于持久化存储 AtomCtx 的文件数据（AGFS 后端）。
...
3. 无需手动创建 Collection，AtomCtx 启动后会自动创建
4. 在配置文件中填写 `storage.vectordb.volcengine.region`，AtomCtx 会自动路由到对应地域的 VikingDB 服务
...
```bash
cp examples/cloud/ctx.conf.example examples/cloud/ctx.conf
```

### 2.2 编辑配置

打开 `examples/cloud/ctx.conf`，将占位符替换为真实值。需要替换的字段如下：
...
| `<your-tos-bucket>` | TOS 桶名称 | 如 `atom_ctxdata` |
...
> **注意**：`ctx.conf` 已被 `.gitignore` 排除，不会被提交到版本库。请妥善保管你的凭据。
...
>
> # 方式 A：放到默认路径
> mkdir -p ~/.ctx && cp examples/cloud/ctx.conf ~/.ctx/ctx.conf
> ctx-server
>
> # 方式 B：通过环境变量指定
> CTX_CONFIG_FILE=examples/cloud/ctx.conf ctx-server
> ```
...
```bash
sudo mkdir -p /etc/ctx
sudo cp ~/.ctx/ctx.conf /etc/ctx/ctx.conf
sudo chmod 600 /etc/ctx/ctx.conf
```

3. 创建 systemd service 文件：

```bash
sudo tee /etc/systemd/system/atom_ctx.service > /dev/null << 'EOF'
[Unit]
Description=AtomCtx Server
After=network.target

[Service]
Type=simple
Environment=CTX_CONFIG_FILE=/etc/ctx/ctx.conf
ExecStart=/usr/local/bin/ctx-server  # 替换为 which ctx-server 的实际输出
Restart=on-failure
RestartSec=5s
...
sudo systemctl start atom_ctx
sudo systemctl status atom_ctx
```

5. 确认服务正常后，设置开机自启（可选）：

```bash
sudo systemctl enable atom_ctx
```

常用管理命令：

```bash
sudo systemctl stop atom_ctx       # 停止服务
sudo systemctl restart atom_ctx    # 重启服务
journalctl -u atom_ctx -f          # 查看实时日志
```
...
> 将 `~/.ctx/ctx.conf` 替换为你实际的配置文件路径。

常用管理命令：

```bash
docker logs atom_ctx        # 查看日志
docker logs -f atom_ctx     # 实时跟踪日志
docker stop atom_ctx        # 停止服务
docker restart atom_ctx     # 重启服务
docker rm -f atom_ctx       # 删除容器（重新 docker run 前需要先删除）
```

**docker compose：**

项目根目录的 `docker-compose.yml` 默认从 `/var/lib/atom_ctx/ctx.conf` 读取配置：

```bash
# 把你的配置文件复制到 docker-compose.yml 期望的路径
sudo mkdir -p /var/lib/atom_ctx
sudo cp ~/.ctx/ctx.conf /var/lib/atom_ctx/ctx.conf

# 在项目根目录下启动（-d 表示后台运行）
docker compose up -d
```

> 如果配置文件不在 `/var/lib/atom_ctx/ctx.conf`，需要修改 `docker-compose.yml` 中 `volumes` 的挂载路径。
...
```bash
helm install atom_ctx ./examples/k8s-helm -f my-values.yaml
```
...
helm install atom_ctx ./examples/k8s-helm \
  --set atom_ctx.config.server.root_api_key="my-strong-secret-key-2024" \
  --set atom_ctx.config.embedding.dense.api_key="YOUR_ARK_API_KEY" \
  --set atom_ctx.config.vlm.api_key="YOUR_ARK_API_KEY" \
  --set atom_ctx.config.storage.vectordb.backend="volcengine" \
  --set atom_ctx.config.storage.vectordb.volcengine.ak="YOUR_AK" \
  --set atom_ctx.config.storage.vectordb.volcengine.sk="YOUR_SK" \
  --set atom_ctx.config.storage.agfs.backend="s3" \
  --set atom_ctx.config.storage.agfs.s3.bucket="atom_ctxdata" \
  --set atom_ctx.config.storage.agfs.s3.access_key="YOUR_AK" \
  --set atom_ctx.config.storage.agfs.s3.secret_key="YOUR_SK" \
  --set atom_ctx.config.storage.agfs.s3.endpoint="https://tos-s3-cn-beijing.volces.com"
```
...
AtomCtx 支持多租户隔离。配置了 `root_api_key` 后自动启用多租户模式。
...
```bash
# 确保 server 已启动，且 root_api_key 与 ctx.conf 一致
...
`status=203/EXEC` 表示 systemd 找不到 `ExecStart` 指定的可执行文件。常见于使用 venv / conda 环境安装 AtomCtx 的情况，`ctx-server` 不在 `/usr/local/bin/` 下。
...
sudo sed -i 's|ExecStart=.*|ExecStart=/实际/路径/ctx-server|' /etc/systemd/system/atom_ctx.service

# 3. 重新加载并启动
sudo systemctl daemon-reload
sudo systemctl restart atom_ctx
sudo systemctl status atom_ctx
```
...
需要在 AtomCtx 项目根目录下执行 `helm install` 命令：
...
- 确认 `atom_ctx.config` 下的 JSON 结构正确（Helm 会将其序列化为 ctx.conf）
