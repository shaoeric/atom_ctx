---
name: ov-add-data
description: This skill adds data(like resources) to AtomCtx Context Database (aka. ov). Use when an agent needs to add files, data from URLs, or external knowledge during interactions. Trigger this tool when 1. is explicitly requested adding files or knowledge; 2. identifies valuable resources worth importing; 3. the user mentioned adding to OV/AtomCtx/Context Database. This skill helps how to use CLI like `ctx add-resource`, `ctx add-skill` and `ctx add-memory` to add resource data, skill files, memory files to AtomCtx.
compatibility: AtomCtx CLI configured at `~/.ctx/ctx-cli.conf`
---

# AtomCtx (OV) `add-resource`

The `ctx add-resource` command imports external resources into AtomCtx's context database — supporting local files, directories, URLs, and remote repositories. Resources are automatically processed with semantic analysis and organized under the `ctx://resources/` namespace.
...
ctx add-resource https://github.com/volcengine/atom-ctx
ctx add-resource https://code.xxxx.org/example/project
ctx add-resource git@code.xxxx.org:example/project.git

# add a file from url
ctx add-resource https://arxiv.org/pdf/2602.09540
ctx add-resource https://raw.githubusercontent.com/volcengine/AtomCtx/main/README.md

# add a file from local filesystem
ctx add-resource ./docs/api-spec.md
ctx add-resource ./team_building.jpg
ctx add-resource /User/volcengine/Documents/profile.pdf
ctx add-resource /User/volcengine/Documents/project.docx

# add a zip file from local filesystem (will be unzipped on server)
ctx add-resource ./docs-of-project.zip

# add a directory from local filesystem
ctx add-resource /User/volcengine/Photo/Travels/2026/ --include "*.jpg,*.jpeg,*.png"
ctx add-resource /User/volcengine/Documents/OV项目设计文档/
```
...
ctx add-resource ./docs --wait

# Wait with timeout (in seconds)
ctx add-resource https://example.com/docs --wait --timeout 60

# Fire and forget (default, without --wait)
ctx add-resource ./docs
```

If you fire and forget, the command will return after the resource is completely downloaded/uploaded on server, and then the CLI will return the root URI of the imported resource. To check the status of the resource, you can use `ctx ls` or `ctx tree` to list the resources under root URI or `ctx://resources/`.

### Specify the target path

By default, resources are imported under `ctx://resources/`. Use `--to` or `--parent` to specify a target uri.

```bash
# The data will be imported as ctx://resources/2026/2026-01-01/, which should not exist before
ctx add-resource /User/volcengine/Photo/Travels/2026/2026-01-01/ --to "ctx://resources/2026/2026-01-01/"

# The data will be imported under ctx://resources/2026, which should exist and be a directory
ctx add-resource /User/volcengine/Photo/Travels/2026/2026-01-02/ --parent "ctx://resources/2026/"
```
...
```
root_uri  ctx://resources/2026/2026-01-01
```

## CLI Prerequisites

- CLI configured in: `~/.ctx/ctx-cli.conf`
- Network access for the importing URL from server
- Local read access for the importing local files/directories from CLI.

# AtomCtx (OV) `add-memory`

The `ctx add-memory` command adds long persistant memory — turning text and structured conversations into searchable, retrievable memories in the AtomCtx context database. Use `ctx add-memory --help` for latest usage.
...
## Input Modes
choose wisely between plain text and multi-turn mode. Multi-turn mode can contain more complex insights, let atom_ctx handle the memory extraction.
...
```bash
ctx add-memory "User's name is Bob, he participate in Global Hackathon in 2025-01-08, and won Champion."
```
...
```bash
ctx add-memory '[
  {"role": "user", "content": "Key facts about the ctx_cli Rust crate"},
  {"role": "assistant", "content": "1. runs faster than python cli\n2. uses HttpClient to connect atom_ctx server\n3. Output formatting supports table and JSON modes\n4. Config lives at ~/.ctx/ctx-cli.conf"}
]'
```
...
# AtomCtx (OV) `add-skill`

The `ctx add-skill` command adds agent capabilities to AtomCtx — supporting SKILL.md files, MCP tool definitions, and raw skill content. Skills are automatically processed and organized under the `ctx://agent/skills/` namespace, making them discoverable and usable by agents.
...
ctx add-skill ./skills/my-skill/SKILL.md

# Add from a directory containing SKILL.md (includes auxiliary files)
ctx add-skill ./skills/my-skill/
```
...
```bash
# Wait until semantic processing finishes
ctx add-skill ./skills/my-skill/ --wait

# Wait with timeout (in seconds)
ctx add-skill ./skills/my-skill/ --wait --timeout 60
...
ctx add-skill ./skills/code-runner/
