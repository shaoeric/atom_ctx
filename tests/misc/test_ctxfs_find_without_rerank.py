...
from atom_ctx_cli.retrieve.types import ContextType, MatchedContext, QueryResult
...
async def test_find_works_without_rerank_config(monkeypatch) -> None:
    ...
    class FakeRetriever:
        ...
        async def retrieve(self, typed_query, ctx, limit, score_threshold, scope_dsl):
            ...
            return QueryResult(
                ...
                matched_contexts=[
                    MatchedContext(
                        uri="ctx://resources/docs/guide.md",
                        context_type=ContextType.RESOURCE,
                        score=0.9,
                    )
                ],
                searched_directories=["ctx://resources/docs"],
            )

    monkeypatch.setattr(
        "atom_ctx.retrieve.hierarchical_retriever.HierarchicalRetriever",
        FakeRetriever,
    )

    result = await fs.find(
        "guide",
        target_uri="ctx://resources/docs",
        limit=3,
        score_threshold=0.2,
        filter={"category": "doc"},
        ctx=request_ctx,
    ...
    assert [ctx.uri for ctx in result.resources] == ["ctx://resources/docs/guide.md"]
    ...
    assert captured["typed_query"].target_directories == ["ctx://resources/docs"]
    ...
    fs._ensure_access.assert_called_once_with("ctx://resources/docs", request_ctx)
