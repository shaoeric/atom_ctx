...
class TestVikingFSURITraversalGuard:
    ...
    @pytest.mark.parametrize(
        "uri",
        [
            "ctx://resources/../_system/users.json",
            "ctx://resources/../../_system/accounts.json",
            "/resources/../_system/users.json",
            "ctx://resources/..\\..\\_system\\users.json",
            "ctx://resources/C:\\Windows\\System32",
        ],
    )
    ...
    async def test_read_file_rejects_traversal_before_agfs_read(self) -> None:
        ...
        with pytest.raises(PermissionError, match="Unsafe URI"):
            await fs.read_file("ctx://resources/../_system/users.json")
    ...
    async def test_write_rejects_traversal_before_agfs_write(self) -> None:
        ...
        with pytest.raises(PermissionError, match="Unsafe URI"):
            await fs.write("ctx://resources/../../_system/accounts.json", "pwned")
    ...
    async def test_rm_rejects_traversal_before_side_effects(self) -> None:
        ...
        with pytest.raises(PermissionError, match="Unsafe URI"):
            await fs.rm("ctx://resources/../../other_account/_system/users.json")
    ...
    @pytest.mark.parametrize(
        ("old_uri", "new_uri"),
        [
            ("ctx://resources/../_system/users.json", "ctx://resources/safe.txt"),
            ("ctx://resources/safe.txt", "ctx://resources/../../victim/_system/users.json"),
        ],
    )
    ...
    async def test_read_file_keeps_valid_uri_behavior(self) -> None:
        ...
        content = await fs.read_file("ctx://resources/docs/guide.md")
