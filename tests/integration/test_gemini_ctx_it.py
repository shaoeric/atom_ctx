...
"""
End-to-end integration tests for AtomCtx add-memory + search using Gemini embeddings.
...
Run:
    GOOGLE_API_KEY=<key> pytest tests/integration/test_gemini_atom_ctx_it.py -v
...
async def test_add_and_search_basic(gemini_ctx_client, tmp_path):
    """Add a single markdown document and verify it is returned by find()."""
    client, model, dim = gemini_ctx_client
...
async def test_batch_documents_search(gemini_ctx_client, tmp_path):
...
async def test_large_text_add_and_search(model, dim, token_limit, tmp_path):
    ...
    data_path = str(tmp_path / "ctx_large")
    Path(data_path).mkdir(parents=True, exist_ok=True)

    client = await make_ctx_client(gemini_config_dict(model, dim), data_path)
    try:
...
async def test_retrieval_routing_workflow(query_param, doc_param, tmp_path):
    ...
    data_path = str(tmp_path / "ctx_routing")
    Path(data_path).mkdir(parents=True, exist_ok=True)

    client = await make_ctx_client(
        gemini_config_dict(
    ...
    try:
        doc = sample_markdown(
            tmp_path,
            "routing_doc",
            "# Retrieval Test\n\nAtomCtx provides memory management for AI agents.",
        )
...
async def test_dimension_variant_add_search(dim, tmp_path):
    ...
    data_path = str(tmp_path / f"ctx_dim_{dim}")
    Path(data_path).mkdir(parents=True, exist_ok=True)

    client = await make_ctx_client(gemini_config_dict("gemini-embedding-2-preview", dim), data_path)
    from atom_ctx_cli.utils.config.ctx_config import AtomCtxConfigSingleton

    assert AtomCtxConfigSingleton.get_instance().embedding.dimension == dim, (
        f"Expected embedder dimension={dim}, got {AtomCtxConfigSingleton.get_instance().embedding.dimension}"
    )
    try:
        doc = sample_markdown(
            tmp_path,
...
async def test_session_search_smoke(gemini_ctx_client, tmp_path):
