...
# AtomCtx CLI Comprehensive Test Script
# This script tests all major AtomCtx CLI commands and scenarios
# Usage: ./test_ctx.sh

set -e

CTX_BIN="./target/release/ctx"
TEST_DIR="/tmp/ctx_test_$$"
mkdir -p "$TEST_DIR"

echo "=========================================="
echo "AtomCtx CLI Comprehensive Test"
echo "=========================================="
echo ""
...
if $CTX_BIN observer queue; then
...
if $CTX_BIN observer system; then
...
# Description: Show and validate AtomCtx configuration
# ============================================================================
print_test "Scenario 2: Configuration Management"
echo "Description: Show and validate AtomCtx configuration"
echo ""

echo "2.1. Show configuration..."
if $CTX_BIN config show; then
...
if $CTX_BIN config validate; then
...
if $CTX_BIN ls "ctx://resources"; then
...
TEST_URI="ctx://resources/test_cli_$$"
if $CTX_BIN mkdir "$TEST_URI"; then
...
if $CTX_BIN tree "ctx://resources"; then
...
if $CTX_BIN stat "ctx://resources"; then
...
if ($CTX_BIN mv "$TEST_URI" "ctx://resources/test_cli_renamed_$$"); then
...
ADD_OUTPUT=$($CTX_BIN add-resource "https://raw.githubusercontent.com/volcengine/AtomCtx/main/README.md" --to "ctx://resources/test_cli_$$" 2>&1)
if echo "$ADD_OUTPUT" | grep -q "root_uri\|success"; then
    ...
    if [ -z "$README_URI" ]; then
        README_URI=$(echo "$ADD_OUTPUT" | grep -o 'ctx://[^[:space:]]*' | head -1)
    fi
...
if $CTX_BIN find "what is AtomCtx" --uri "ctx://resources" --limit 5; then
...
if $CTX_BIN search "context database" --uri "ctx://resources" --limit 5; then
...
if $CTX_BIN grep "ctx://resources" "AtomCtx"; then
...
if $CTX_BIN glob "*.md" --uri "ctx://resources"; then
...
SESSION_OUTPUT=$($CTX_BIN session new 2>&1)
...
if [ -n "$SESSION_ID" ]; then
    ...
    if $CTX_BIN session get "$SESSION_ID"; then
    ...
    if $CTX_BIN session add-message "$SESSION_ID" --role "user" --content "What is AtomCtx?"; then
    ...
    if $CTX_BIN session commit "$SESSION_ID"; then
    ...
    if $CTX_BIN session delete "$SESSION_ID"; then
...
if [ -n "$README_URI" ]; then
    ...
    if $CTX_BIN link "$README_URI" "ctx://resources/test" --reason "test relation"; then
    ...
    if $CTX_BIN relations "$README_URI"; then
    ...
    if $CTX_BIN unlink "$README_URI" "ctx://resources/test"; then
...
# Description: Test export and import of .ctxpack files
# Note: Command syntax is 'ctx export <URI> <TO>' (not --to)
# ============================================================================
print_test "Scenario 8: Pack Operations"
echo "Description: Test export and import of .ctxpack files"
echo ""

if [ -n "$README_URI" ]; then
    PACK_FILE="$TEST_DIR/test.ctxpack"
    PARENT_URI=$(dirname "$README_URI")

    echo "8.1. Export to .ctxpack..."
    if $CTX_BIN export "$PARENT_URI" "$PACK_FILE"; then
    ...
    if [ -f "$PACK_FILE" ]; then
        echo "8.2. Import from .ctxpack..."
        IMPORT_URI="ctx://resources/test_import_$$"
        if $CTX_BIN import "$PACK_FILE" "$IMPORT_URI" --force; then
