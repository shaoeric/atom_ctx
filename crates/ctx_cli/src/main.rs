...
#[derive(Parser)]
#[command(name = "atom_ctx")]
#[command(about = "AtomCtx - An Agent-native context database")]
#[command(version = env!("CTX_CLI_VERSION"))]
...
struct Cli {
    ...
    /// Account identifier to send as X-AtomCtx-Account
    #[arg(long, global = true)]
    account: Option<String>,

    /// User identifier to send as X-AtomCtx-User
    #[arg(long, global = true)]
    user: Option<String>,

    /// Agent identifier to send as X-AtomCtx-Agent
...
enum Commands {
    ...
    /// Export context as .ctxpack
    Export {
        /// Source URI
        uri: String,
        /// Output .ctxpack file path
        to: String,
    },
    /// Import .ctxpack into target URI
    Import {
        /// Input .ctxpack file path
        file_path: String,
        /// Target parent URI
        target_uri: String,
    ...
    #[command(alias = "list")]
    Ls {
        /// Ctx URI to list (default: ctx://)
        #[arg(default_value = "ctx://")]
        uri: String,
    ...
    Grep {
        /// Target URI
        #[arg(short, long, default_value = "ctx://")]
        uri: String,
        /// Search pattern
        pattern: String,
        /// Case insensitive
    ...
    Tui {
        /// Ctx URI to start browsing (default: /)
        #[arg(default_value = "/")]
        uri: String,
    },
    /// Chat with ctxbot agent
    Chat {
        ...
        if !path_obj.exists() {
                ...
                args.iter().position(|s| s == "add-resource" || s == "add")
            {
                if args.len() > add_resource_pos + 2 {
                    ...
                    eprintln!("Suggested command: ctx add-resource \"{}\"", suggested_path);
                }
            }
...
async fn handle_find(
    ...
    print_command_echo("ctx find", &params.join(" "), ctx.config.echo_command);
    let client = ctx.get_client();
...
async fn handle_search(
    ...
    params.push(format!("\"{}\"", query));
    print_command_echo("ctx search", &params.join(" "), ctx.config.echo_command);
    let client = ctx.get_client();
...
async fn handle_ls(
    ...
    print_command_echo("ctx ls", &params.join(" "), ctx.config.echo_command);
...
async fn handle_tree(
    ...
    print_command_echo("ctx tree", &params.join(" "), ctx.config.echo_command);
...
async fn handle_grep(
    ...
    print_command_echo("ctx grep", &params.join(" "), ctx.config.echo_command);
    let client = ctx.get_client();
...
async fn handle_glob(pattern: String, uri: String, node_limit: i32, ctx: CliContext) -> Result<()> {
    ...
    print_command_echo("ctx glob", &params.join(" "), ctx.config.echo_command);
    let client = ctx.get_client();
